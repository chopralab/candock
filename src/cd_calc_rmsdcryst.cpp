...
int main(int argc, char* argv[]) {
    try {
        if (argc <= 2) {
            ...
            cerr << "A third argument gives the number of threads (defaults to "
                    "1)"
                 << endl;
        ...
        if (cryst.size() != 1) {
            cerr << "The crystal ligand file must only have 1 molecule in it"
                 << endl;
            return 1;
        }
        ...
        molib::Atom::Graph cryst_graph =
            molib::Atom::create_graph(cryst[0].get_atoms());
        ...
        for (size_t thread_id = 0; thread_id < num_threads; ++thread_id)
            threads.push_back(std::thread([&, thread_id] {
                for (size_t i = thread_id; i < mols1.size(); i += num_threads) {
                    ...
                    rmsd_graph[i] =
                        molib::Atom::compute_rmsd(cryst_graph, atom_graphs[i]);

                    molib::Atom::Vec curr_atoms = mols1[i].get_atoms();
                    rmsd_vinas[i] = std::max(
                        molib::compute_rmsd_vina(cryst_atoms, curr_atoms),
