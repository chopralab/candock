    ...
    try {
        ...
        if (cmdl.get_string_option("obj_dir").empty()) {
            ...
            score.parse_objective_function(cmdl.get_string_option("obj_dir"),
                                           cmdl.get_double_option("scale"), 15);
        }
        ...
        for (size_t i = 0; i < receptor_mols.size(); ++i) {
            ...
            OMMIface::Modeler modeler(ffield, cmdl.get_string_option("fftype"),
            ...
            const double energy =
                score.non_bonded_energy(gridrec_min, minimized_ligand);
            modeler.mask(protein.get_atoms());
            const double potential = modeler.potential_energy();

            fileout::print_complex_pdb(std::cout, minimized_ligand,
                                       minimized_receptor, energy, potential,
                                       i + 1, 0xFFFFFF, std::nan(""));
