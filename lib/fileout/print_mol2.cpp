...
const std::map<const std::string, const std::string> id_atom_to_sybyl{
    {"C3", "C.3"},   {"C2", "C.2"},  {"Cac", "C.2"},   {"Car", "C.ar"},
    {"C1", "C.1"},   {"HC", "H"},    {"N3+", "N.3"},   {"N3", "N.3"},
    {"N2", "N.2"},   {"N1", "N.1"},  {"Ntr", "N.2"},   {"O3", "O.3"},
    {"O3-", "O.3"},  {"O2", "O.2"},  {"O2-", "O.2"},   {"S3", "S.3"},
    {"Nar", "N.ar"}, {"Pac", "P.3"}, {"Pax", "P.3"},   {"P", "P.3"},
    {"S3", "S.3"},   {"S2", "S.2"},  {"Npl", "N.pl3"}, {"???", "ANY"},
};
...
void print_mol2(std::ostream& ss, const molib::Molecule& ligand) {
    ...
    for (size_t i = 0; i < all_atoms.size(); ++i) {
        ...
        if (id_atom_to_sybyl.count(all_atoms[i]->idatm_type_unmask()) != 0) {
            ss << std::setw(5)
               << id_atom_to_sybyl.at(all_atoms[i]->idatm_type_unmask());
        } else {
            ss << std::setw(5) << all_atoms[i]->idatm_type_unmask();
        }

        ss << " 1 ";
        ss << all_atoms[i]->br().resn() +
                  std::to_string(all_atoms[i]->br().resi());
        ss << " 0.0000\n";
    }
