/* This is genlig.cpp and is part of CANDOCK
                ...
                if (assembly.number() == stoi(lc[4])) {
                    for (auto& model : assembly) {
                        if (model.number() == stoi(lc[3])) {
                            ...
                            if (model.remarks(7, molib::Residue::res_tuple2(
                                                     lc[2].at(0), lc[0],
                                                     stoi(lc[1]), ' '),
                                              respair)) {
                                for (auto& rp : respair) {
...
molib::Molecule get_ligand(
    ...
    const string& lig_code) {  // output rotated binding site residues
                ...
                for (auto& chain : model) {
                    for (auto& residue : chain) {
                        ...
                        if (lig_code.find(lresn + ":" + to_string(lresi) + ":" +
                                          chain.chain_id() + ":" +
                                          to_string(model.number()) + ":" +
                                          to_string(assembly.number()) + ":" +
                                          molecule.name() + ":" +
                                          mols.name()) != string::npos) {
                            if (chain.chain_id() != chain_id) {
                                plchain = &lmodel.add(
                                    new molib::Chain(chain.chain_id()));
                                chain_id = chain.chain_id();
                            }
                            molib::Chain& lchain = *plchain;
                            lchain.add(new molib::Residue(residue));
                            dbgmsg(lresn + ":" + to_string(lresi) + ":" +
                                   chain.chain_id() + ":" +
                                   to_string(model.number()) + ":" +
                                   to_string(assembly.number()) + ":" +
...
molib::Molecule mark(const ResidueSet& neighbor_residues,
                     const ResSet& aligned_part_of_binding_site,
...
//~ molib::Atom::Grid parse_receptor(const string &receptor_file, const string
//&receptor_chain_id) {
...
//~ }
void generate_ligands(const string& receptor_file,
                      const string& receptor_chain_id, const string& json_file,
                      const string& bio_dir, const string& lig_code,
    ...
    molib::Atom::Grid gridrec(
        query_mols[0].get_atoms(receptor_chain_id, molib::Residue::protein));

    JsonReader jr;
    molib::NRset nrset;
    // read bio PDBs
    //~ molib::PDBreader
    //biopdb(molib::PDBreader::all_models|molib::PDBreader::hydrogens);
    // the aligned files from json
    jr.parse_JSON(json_file);
    for (auto& d : jr.root()) {
        try {  // if something goes wrong, e.g., pdb file is not found, don't
               // exit..
            ...
            const string pdb_file =
                Path::join(bio_dir, pdb_id + chain_ids + ".pdb");
...
#endif
                ResSet aligned_part_of_binding_site =
                    find_binding_site(mols, lig_code, aligned_to_query);
                molib::Molecules ligands;
                ligands.add(new molib::Molecule(get_ligand(mols, lig_code)));
                dbgmsg(ligands);
                ResidueSet neighbor_residues =
                    find_neighbor_residues(ligands.first(), gridrec);
...
//~ void generate_ligands(const string &receptor_file, const string
//&receptor_chain_id,
...
//~
//~ }
pair<BindingSiteClusters, BindingSiteScores> generate_binding_site_prediction(
    const string& json_with_ligs_file, const string& bio_dir,
    ...
    for (auto& d : jr.root()) {
        ...
        for (size_t i = 0; i < hetero.size();
             ++i) {  // go over the hetero ligands only
            dbgmsg(hetero[static_cast<int>(i)].asString());
            const vector<string> ligand =
                help::ssplit(hetero[static_cast<int>(i)].asString(), ":");
            //	dbgmsg(ligand.size());
            ...
            // lig_code =
            // resn:resi:chain_id:model_num:assembly_num:molecule_name
    ...
    for (auto& kv1 : ligands_by_bio_file) {
        ...
        try {  // if something goes wrong, e.g., pdb file is not found, don't
               // exit..
            //~ biopdb.rewind();
            parser::FileParser biopdb(bio_file, parser::all_models);
            molib::Molecules mols = biopdb.parse_molecule();
            mols.rotate(mx, true);  // inverse rotation
            for (auto& linf : kv1.second) {
                if (linf.cluster_number <=
                    num_bsites) {  // trim number of binding sites
                    bsites[linf.cluster_number].add(
                        new molib::Molecule(get_ligand(mols, linf.lig_code)));
                    bscores[linf.cluster_number] =
                        max(bscores[linf.cluster_number], linf.z_score);
                }
            }
        } catch (exception& e) {
            log_warning << e.what() << " ... skipping ... " << endl;
