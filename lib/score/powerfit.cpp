...
void ex_rep_callback(const size_t iter, void* unused,
    ...
    dbgmsg("iter: " << iter << " sigma = " << gsl_vector_get(params, 0)
...
/* solve the system with a maximum of 20 iterations */
...
#else
    status = gsl_multifit_nlinear_driver(max_iter, xtol, gtol, ftol, NULL, NULL,
                                         &info, w);
    ...
    dbgmsg("summary from method '" << gsl_multifit_nlinear_name(w) << "/"
    ...
    dbgmsg("sigma  = " << sigma << " +/- "
                       << c * std::sqrt(gsl_matrix_get(covar, 0, 0)));
    dbgmsg("b      = " << b << " +/- "
                       << c * std::sqrt(gsl_matrix_get(covar, 1, 1)));
    ...
    for (double b_guess = -2.00; b_guess <= 2; b_guess += 0.50)
        for (size_t p_guess = 9; p_guess <= 13; p_guess += 1)
            for (double s_guess = 1e+5; s_guess < 1e+10; s_guess *= 2) {
                ...
                try {
                    std::tie(sigma, b, chi) = fit_power_function(
                        x, y, p_guess, std::make_pair(s_guess, b_guess), 100);
                } catch (Error& e) {
                    continue;
                }
...
std::tuple<double, double, double> fit_range_power_function_fast(
    ...
    if (best_error == HUGE_VAL) {
        throw InterpolationError(
            "warning : could not fit repulsion term, zero everything");
    }
