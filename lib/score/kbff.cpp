...
KBFF &KBFF::parse_objective_function(const string &obj_dir, const double scale_non_bond, const size_t max_step)
{
...
KBFF &KBFF::compile_objective_function(const double scale_non_bond)
{
    ...
    Benchmark bench;
    auto energy_function = __ref_state == "mean" ? mem_fn(&KBFF::__energy_mean) : mem_fn(&KBFF::__energy_cumulative);
    for (auto &el1 : __gij_of_r_numerator)
    {
        ...
        for (size_t i = start_idx; i < energy.size() - 2; ++i)
        {
            ...
            {
                size_t j = i + 1;
                while (j < i + 3 && j < energy.size() && (energy[j] == 0 || energy[j] == 5))
                {
                    ++j;
                }
                ...
                for (size_t i = start_idx; i < end_idx && i < energy.size(); ++i)
                {
                    if (energy[i] != -HUGE_VAL)
                    {
                        if (energy[i] < global_min)
                        {
                            ...
                            repulsion_idx = i;
                            while (repulsion_idx > 0 && energy[repulsion_idx] != -HUGE_VAL && ((energy[repulsion_idx] - energy[repulsion_idx - 1]) / __step_in_file) < 0.75)
                                --repulsion_idx;
                        }
                    }
                }
        ...
        try
        {
            ...
            for (size_t i = repulsion_idx; i < energy.size(); ++i)
            { // unset everything below this value
