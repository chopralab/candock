...
ostream& operator<<(ostream& os, const BondToOrder& bond_orders) {
    for (auto& kv : bond_orders) {
        ...
        os << "bond_order(" << b.atom1().atom_number() << ","
           << b.atom2().atom_number() << ") = " << kv.second << endl;
    }
    return os;
}
...
// for each valence state determine bond orders (boaf procedure)
        ...
        for (auto& valence_state : valence_states) {
            ...
            try {
                ...
                if (__basic_rules(valence_state, bond_orders)) {
                    ...
                    dbgmsg("MOLECULE AFTER COMPUTING BOND ORDERS" << endl
                                                                  << atoms);
                    return;
                }
        ...
        // if boaf fails for all saved valence states, a warning message is
        // given
        throw Error("die : bond order assignment failed");
    } catch (exception& e) {
        log_error << "errmesg : " << e.what() << " for residue = " << endl
                  << atoms << endl;
        throw e;
    }
}
    ...
    for (auto a : bonds) {
        ...
        if (atom_bonds.size() != 4) {
            throw BondOrderError(
                "Error assigning chirality, wrong number of neighbors");
        }

        // FIXME: Using the topologic order is a bad idea, but it's all I got
        // for now
        Atom::Vec neighbors;
        ...
        double improper_1 =
            geometry::dihedral(neighbors[0]->crd(), a->crd(),
                               neighbors[1]->crd(), neighbors[2]->crd());
        double improper_2 =
            geometry::dihedral(neighbors[0]->crd(), a->crd(),
                               neighbors[1]->crd(), neighbors[3]->crd());
...
void BondOrder::__dfs(const int level, const int sum, const int tps,
                      const vector<vector<AtomParams>>& V,
                      vector<AtomParams>& Q,
    ...
    for (auto& p : params) {
        if (p.aps + sum <= tps) {
            ...
            if (level + 1 < static_cast<int>(V.size())) {
                __dfs(level + 1, p.aps + sum, tps, V, Q, valence_states,
                      max_valence_states);
...
ValenceStateVec BondOrder::__create_valence_states(
    const Atom::Vec& atoms, const int max_valence_states) {
#ifndef NDEBUG
    Benchmark bench;
#endif
    vector<vector<AtomParams>>
    ...
    for (auto& patom : atoms) {
        ...
        sort(aps.begin(), aps.end(),
             [](const AtomParams& i, const AtomParams& j) {
                 return i.aps < j.aps;
             });
#ifndef NDEBUG
    ...
    // (issue #103)
    sort(valence_states.begin(), valence_states.end(),
         [](const vector<AtomParams>& i, const vector<AtomParams>& j) {
             int tps_i = 0;
             for (auto& e : i) tps_i += e.aps;
             int tps_j = 0;
             for (auto& e : j) tps_j += e.aps;
             const AtomParams max_ap_i =
                 *max_element(i.begin(), i.end(),
                              [](const AtomParams& iap, const AtomParams& jap) {
                                  return iap.aps < jap.aps;
                              });
             const AtomParams max_ap_j =
                 *max_element(j.begin(), j.end(),
                              [](const AtomParams& iap, const AtomParams& jap) {
                                  return iap.aps < jap.aps;
                              });
             if (tps_i == tps_j)
...
bool BondOrder::__discrepancy(const ValenceState& valence_state) {
    ...
    for (auto& kv : valence_state) {
        ...
        if ((apar.val != 0 && apar.con == 0) ||
            ...
            dbgmsg("Failure for atom = " << kv.first->atom_name() << " val = "
                                         << apar.val << " con = " << apar.con
                                         << " aps = " << apar.aps);
            return true;
        }
    }
...
bool BondOrder::__basic_rules(ValenceState& valence_state,
                              BondToOrder& bond_orders) {
    while (!__my_success(valence_state)) {
            ...
            if (apar.con == apar.val) {
                for (auto& pbond : atom.get_bonds()) {
                    ...
                    if (!bond_orders.count(&bond)) {  // unassigned bond order
                        // rule 1 : for each atom in a bond, if the bond order
                        // bo
                        // is determined, con is deducted by 1 and av is
                        // deducted by bo
                        bond_orders[&bond] = 1;
                        ...
                        AtomParams& apar2 =
                            valence_state.at(&bond.second_atom(atom));
                        apar.con -= 1;
                        apar.val -= 1;
                        apar2.con -= 1;
                        apar2.val -= 1;
            ...
            // order of the last bond is set to av
                ...
                for (auto& pbond : atom.get_bonds()) {
                    ...
                    if (!bond_orders.count(&bond)) {  // unassigned bond order
                        ...
                        // rule NEW (Janez) : if trying to assign a zero bond
                        // order then
        ...
        // b) if discrepancy happens (av is not 0 when con is 0 or av is 0 when
        // con is not 0)
        // reset the bond order to 2 and then 3.
        if (__discrepancy(valence_state)) return false;
...
Bond& BondOrder::__get_first_unassigned_bond(const ValenceState& valence_state,
                                             BondToOrder& bond_orders) {
    for (auto& kv : valence_state) {
        const Atom& atom = *kv.first;
        for (auto& pbond : atom.get_bonds()) {
...
void BondOrder::__trial_error(ValenceState& valence_state,
                              BondToOrder& bond_orders) {
    for (int bo = 1; bo <= 3; ++bo) {
        // save valence state
        ValenceState saved = valence_state;
        BondToOrder saved_bond_orders = bond_orders;
        // a) assume bond order (of one randomly selected bond) is 1, then
        // continue
    ...
    // c) if discrepancies happen for all 3 bond orders, boaf exits and returns
    // 0
    throw BondOrderError(
