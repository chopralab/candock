...
void Internal::build(const Atom::Vec& atoms) {
    for (auto& pa1 : atoms) {  // find all bonds lengths
        ...
        for (auto& a2 : a1) {
            ...
            dbgmsg("bond length between atoms " << a1.atom_number() << " and "
                                                << a2.atom_number() << " is "
                                                << __ic_bond[&a1][&a2]);
            for (auto& a3 : a2) {  // find all angles
                if (&a1 != &a3) {
                    __ic_angle[&a1][&a2][&a3] =
                        geometry::angle(a1.crd(), a2.crd(), a3.crd());
                    dbgmsg("angle between atoms "
                           << a1.atom_number() << ", " << a2.atom_number()
                           << " and " << a3.atom_number() << " is "
                           << geometry::degrees(__ic_angle[&a1][&a2][&a3]));
                    for (auto& a4 : a3) {  // find all torsional angles
                        if (&a1 != &a4 && &a2 != &a4) {
                            __ic_dihedral[&a1][&a2][&a3][&a4] =
                                geometry::dihedral(a1.crd(), a2.crd(), a3.crd(),
                                                   a4.crd());
                            dbgmsg("dihedral between atoms "
                                   << a1.atom_number() << ", "
                                   << a2.atom_number() << ", "
                                   << a3.atom_number() << " and "
                                   << a4.atom_number() << " is "
...
geometry::Coordinate Internal::__set_crd(const geometry::Point& p1,
                                         const geometry::Point& p2,
                                         const geometry::Point& p3,
                                         const double bond, const double angle,
                                         const double dihedral) const {
    ...
    return p3 - v2n * (bond * cos(angle)) - n2 * (yy * cos(dihedral)) +
           n * (yy * sin(dihedral));
}

geometry::Point::Vec Internal::cartesian(const Atom& ini1, const Atom& ini2,
                                         const Atom& ini3,
                ...
                if (atoms.count(const_cast<Atom*>(a4))) {
                    if (!new_crd.count(a4)) {
                        new_crd.insert(
                            {a4,
                             this->__set_crd(
                                 new_crd[a1], new_crd[a2], new_crd[a3],
                                 __ic_bond.at(a3).at(a4),
                                 __ic_angle.at(a2).at(a3).at(a4),
                                 __ic_dihedral.at(a1).at(a2).at(a3).at(a4))});
                        dbgmsg("coords of atom "
                               << a4->atom_number() << " are " << a4->crd()
                               << " bond length between " << a3->atom_number()
                               << " and " << a4->atom_number() << " is "
                               << __ic_bond.at(a3).at(a4) << " angle between "
                               << a2->atom_number() << " and "
                               << a3->atom_number() << " and "
                               << a4->atom_number() << " is "
                               << __ic_angle.at(a2).at(a3).at(a4)
                               << " dihedral between " << a1->atom_number()
                               << " and " << a2->atom_number() << " and "
                               << a3->atom_number() << " and "
                               << a4->atom_number() << " is "
                               << __ic_dihedral.at(a1).at(a2).at(a3).at(a4));
