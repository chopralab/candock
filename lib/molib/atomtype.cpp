            ...
            if (ring_bonds.count(&b2)) {
                for (auto& b3 : b2) {
                    if (ring_bonds.count(&b3) && &b3 != &b1) {
                        const double dangle =
                            geometry::degrees(geometry::dihedral(
                                b1.atom1().crd(), b1.atom2().crd(),
                                b3.atom1().crd(), b3.atom2().crd()));

                        dbgmsg("dihedral angle is "
                               << dangle << " between atoms "
                               << b1.atom1().atom_number() << ", "
                               << b1.atom2().atom_number() << ", "
                               << b3.atom1().atom_number() << ", "
                               << b3.atom2().atom_number());

                        // larger than 10 degrees, means NOT aromatic
                        if (fabs(dangle) > 10 &&
                            fabs(fabs(dangle) - 180) > 10) {
                            return false;
                        }
                    }
                }
...
bool aromatic(const Atom::Set& ring) {
    ...
    if (homas < 0.271) {
        ...
        if (check_dihedrals_for_planarity(ring_bonds)) is_aromatic = true;
    }
    return is_aromatic;
}
void compute_idatm_type(const Atom::Vec& atoms) {
    ...
    for (auto& pa : atoms) pa->set_idatm_type("???");
    ...
    for (auto& pa : atoms) {
        ...
        if (element.number() == 1) {
            ...
            a.set_idatm_type(bondedToCarbon ? (isHyd ? "HC" : "DC")
                                            : (isHyd ? "H" : "D"));
            dbgmsg("hydrogens" << boolalpha
                               << (bondedToCarbon ? (isHyd ? "HC" : "DC")
                                                  : (isHyd ? "H" : "D")));
        }
        ...
        heavys[&a] = heavyCount;
        dbgmsg("pass 1  : " << a.idatm_type_unmask() << " heavys["
                            << a.atom_name() << "] = " << heavys[&a]);
    }

    // "pass 1.5": use templates for "infallible" typing of standard residue
    // types
        ...
        if (it != help::standard_residues.end()) {
            for (auto& a : residue) {
                if (!mapped[&a]) {
                    ...
                    // H on peptide N
                    if (a.atom_name() == "HN") idatm_type = "H";
                    // still not known? find it in the table!
                    if (idatm_type == "???") {
                        auto it2 = it->second.find(a.atom_name());
                        if (it2 == it->second.end())
                            throw Error(
                                "die : cannot find atom name " + a.atom_name() +
                                " of template residue " + residue.resn());
                        idatm_type = it2->second;
                    }
                    a.set_idatm_type(idatm_type);
                    mapped[&a] = true;
                }
                dbgmsg("pass 1.5  : " << a.idatm_type_unmask() << " "
                                      << a.atom_name() << " "
                                      << a.atom_number());
            }
        }
    ...
    for (auto& pa : atoms) {
        ...
        //		or S3+
            ...
            for (int n1 = 0; n1 < 3; ++n1) {
                for (int n2 = n1 + 1; n2 < 3; ++n2) {
                    avgAngle += geometry::degrees(
                        geometry::angle(a[n1].crd(), a.crd(), a[n2].crd()));
                }
            }
            avgAngle /= 3.0;
            ...
            if (element == Element::C) {
                if (avgAngle < angle23val1)
                    ...
                    a.set_idatm_type(freeOxygens(a, heavys) >= 2 ? "Cac"
                                                                 : "C2");
            } else if (element == Element::N) {
                if (avgAngle < angle23val1)
                    a.set_idatm_type("N3");
                else
                    a.set_idatm_type(freeOxygens(a, heavys) >= 2 ? "Ntr"
                                                                 : "Npl");
            } else if (element == Element::S) {
                bool hasOxy = false;
                for (int i = 0; i < 3; ++i) {
        ...
        //	O and S are sp3 (O3 and S3, respectively)
            ...
            double ang = geometry::degrees(
                geometry::angle(a[0].crd(), a.crd(), a[1].crd()));
            dbgmsg("valence 2 typing for atom = "
                   << a.atom_number() << " angle(" << a[0].atom_number() << ","
                   << a.atom_number() << "," << a[1].atom_number()
                   << ") = " << ang);
            if (element == Element::C) {
                if (ang < angle23val1) {
                    a.set_idatm_type("C3");
    ...
    for (auto& pa : atoms) {
        ...
        if (a.idatm_type_unmask() == "C") {
            ...
            if (sqlen <= p3c1c1 && bondeeType == "C1") {
                a.set_idatm_type("C1");
            } else if (sqlen <= p3c2c && bondee.element() == Element::C) {
                a.set_idatm_type("C2");
            } else if (sqlen <= p3c2n && bondee.element() == Element::N) {
                a.set_idatm_type("C2");
            } else {
                a.set_idatm_type("C3");
            }
            ...
            if (bondeeType == "Cac" || bondeeType == "Pac" ||
                bondeeType == "Sac" || bondeeType == "Ntr") {
                if (!mapped[&a])
                    //~ a.set_idatm_type("O-");
                    a.set_idatm_type("O2-");
            } else if (bondeeType == "Nox" || bondeeType == "Pox" ||
                       bondeeType == "Son" || bondeeType == "Sxd") {
                if (!mapped[&a]) a.set_idatm_type("O2");
            } else if (sqlen <= p3o2c2 && bondee.element() == Element::C) {
                if (!mapped[&a]) a.set_idatm_type("O2");
                if (!mapped[&bondee]) bondee.set_idatm_type("C2");
                redo[&bondee] = 0;
            } else if (sqlen <= p3o2as && bondee.element() == Element::As) {
                if (!mapped[&a]) a.set_idatm_type("O2");
            } else {
                if (!mapped[&a]) a.set_idatm_type("O3");
            }
        } else if (a.idatm_type_unmask() == "S") {
            if (bondee.element() == Element::P) {
                if (!mapped[&a]) a.set_idatm_type("S2");
            } else if (sqlen <= p3s2c2 && bondee.element() == Element::C) {
                if (!mapped[&a]) a.set_idatm_type("S2");
                if (!mapped[&bondee]) bondee.set_idatm_type("C2");
                redo[&bondee] = 0;
            } else if (sqlen <= p3s2as && bondee.element() == Element::As) {
                if (!mapped[&a]) a.set_idatm_type("S2");
            } else {
                if (!mapped[&a]) a.set_idatm_type("S3");
            }
        }
    ...
    for (auto& pa : atoms) {
        Atom& a = *pa;
        if (mapped[&a]) redo[&a] = 0;

        if (redo[&a] == 0) continue;
        ...
        for (auto& bondee : a) {
            ...
            if (redo[&a] == 1) {
                if ((sqlen > p4c3c && bondeeElement == Element::C) ||
                    (sqlen > p4c3n && bondeeElement == Element::N) ||
                    (sqlen > p4c3o && bondeeElement == Element::O)) {
                    a.set_idatm_type("C3");
                    break;
                }
                if ((sqlen <= p4c2c && bondeeElement == Element::C) ||
                    (sqlen <= p4c2n && bondeeElement == Element::N)) {
                    a.set_idatm_type("C2");
                }
            } else if (redo[&a] == 2) {
                if ((sqlen <= p4n2c && bondeeElement == Element::C) ||
                    (sqlen <= p4n2n && bondeeElement == Element::N)) {
                    a.set_idatm_type("Npl");
                    break;
                }
            } else {
    ...
    for (auto& pa : atoms) {
        Atom& a = *pa;
        if (a.idatm_type_unmask() != "C2") continue;
        ...
        if (!c2possible) a.set_idatm_type("C3");
    ...
    for (auto& pa : atoms) {
        ...
        if (a.idatm_type_unmask() == "N3") {
            ...
            for (auto& bondee : a) {
                ...
                if (bondeeType != "C3" && bondeeType != "H" &&
                    bondeeType != "D") {
                    positive = false;
                    break;
                }
            }
            if (positive) a.set_idatm_type("N3+");

        } else if (a.idatm_type_unmask() == "C2") {
            int numNpls = 0;
            for (auto& bondee : a) {
                if ((bondee.idatm_type_unmask() == "Npl" && !mapped[&bondee]) ||
            ...
            if (numNpls == 3) {
                for (auto& bondee : a) {
                    if (bondee.idatm_type_unmask() != "Npl") continue;
                    if (mapped[&bondee]) continue;

                    bondee.set_idatm_type("Ng+");
                    for (auto& bondee2 : bondee) {
                        const string bondee2type = bondee2.idatm_type_unmask();
                        if ((bondee2type == "C2" || bondee2type == "Npl") &&
                            &bondee2 != &a) {
            ...
            for (auto& r : rings) {
                ...
                for (auto& pa : r) {
                    ...
                    a.add_property("ag" + std::to_string(r.size()))
                        .add_property("ag");
                    if (a.idatm_type_unmask() == "C2") a.set_idatm_type("Car");
                    if (a.idatm_type_unmask() == "C3") a.set_idatm_type("Car");
                    if (a.idatm_type_unmask() == "O3") a.set_idatm_type("Oar");
                    if (a.idatm_type_unmask() == "S3") a.set_idatm_type("S2");
                    if (a.idatm_type_unmask() == "N3") a.set_idatm_type("Npl");
    ...
    for (auto& pa : atoms) {
        ...
        if (a.idatm_type_unmask() != "Npl") continue;

        if (heavys[&a] != 2) continue;
        ...
        for (auto& bondee : a) {
            ...
            if (idatmType != "C3" && idatmType != "N3+" && idatmType != "N3" &&
                idatmType != "O3" && idatmType != "S3+" && idatmType != "S3" &&
                idatmType != "P3+") {
                bothSP3 = false;
        ...
        avgLen /= 2.0;
        if (bothC) {
            if (avgLen <= p8cc2n2h) a.set_idatm_type("N2");
        } else if (bothN) {
            if (avgLen <= p8nn2n2h) a.set_idatm_type("N2");
    ...
    // "pass 9": change O2- to O3- for sulfates, phosphates, N-oxide, S3- for
    // thiophosphate ...
    for (auto& patom : atoms) {
        const Residue& residue = patom->br();
        if (!help::standard_residues.count(residue.resn())) {
            Fragmenter(residue.get_atoms()).substitute_atoms(help::special);
            dbgmsg("pass 9 (renaming special atom types) : " << endl
                                                             << residue);
        }
    }
    // "pass 10": check if idatm types and SYBYL atom types (if available) are
    // in sync (issue #109)
    for (auto& pa : atoms) {
        Atom& a = *pa;
        if (mapped[&a]) continue;
 ...
 * (as atoms argument) then it should be checked before calling this function
 * whether
...
void compute_ring_type(const Atom::Vec& atoms) {
    ...
    for (auto& ring : rings) {
        ...
        if (aromatic(ring)) {
            int num_c = 0, num_n = 0;
            for (auto& pa : ring) {
                if (pa->idatm_type_unmask() == "Car")
                    num_c++;
                else if (pa->idatm_type_unmask() == "N2" ||
                         pa->idatm_type_unmask() == "N2+")
                    num_n++;
            }
        ...
        for (auto& pa : ring) {
            pa->add_property(ring_type).add_property(aromatic_type);
        }
    }
