...
void MNTS::initialize() {
    ...
    adjacMatrix =
        unique_ptr<unique_ptr<int[]>[]>(new unique_ptr<int[]>[Max_Vtx]);
...
void MNTS::output() {
    ...
    //~ for( i = 0; i < 100; i++ ) {
    for (i = 0; i < iter; i++) {
        ss << "sum = " << W_used[i] << ", iter = " << Iteration[i]
           << ", len = " << len_used[i] << ", qmax = ";
    ...
    int count = 0;
    ss << "\n The best weight value for the maximum weighted problem is "
       << best_v << endl;
    //~ for( i = 0; i < 100; i++ ) {
    for (i = 0; i < iter; i++) {
        wavg = wavg + W_used[i];
    ...
    ss << "avg_sum = " << twavg << ", succes = " << count << ", len = " << lenbb
       << ", avg_iter = " << iteravg << endl;
    dbgmsg(ss.str());
}
void MNTS::max_tabu(int ii) {
    ...
    for (i = 0; i < len_time; i++) {
        l = tabu(len_improve);
        //~ dbgmsg("i = " << i << " l = " << l << " lbest = " << lbest << " Waim
        //= " << Waim << " len_improve = " << len_improve << " len_time = " <<
        //len_time);
        M_iter = M_iter + Iter;
        if (l > lbest) {
            lbest = l;
    ...
    dbgmsg("i = " << ii << " l = " << lbest);
    //~ return lbest;
    //~ cout << "time to find one max.weight clique " <<
