...
namespace Glib {
	...
	public:
		...
		Graph1 &GetGraph1() { return g1; }
		Graph2 &GetGraph() { return g2; }
	...
	UllSubState<Graph1, Graph2>::UllSubState(Graph1 &ag1, Graph2 &ag2) : core_len(0), core_1(ag1.size(), NULL_NODE), core_2(ag2.size(), NULL_NODE), g1(ag1), g2(ag2), n1(ag1.size()), n2(ag2.size()) { 
		...
		for (int i=0; i<n1; i++)
			for(int j=0; j<n2; j++) {
				//~ M[i][j]=(g1.get_vertex(i).get_edge().size() <= g2.get_vertex(j).get_edge().size() && g1.get_vertex(i).compatible(g2.get_vertex(j)) ? 1 : 0;
				//~ M[i][j]=(g1[i].size() <= g2[j].size() && g1[i].compatible(g2[j]) ? 1 : 0);
				// need to use function "get_num_edges" to get the real number of edges, because g1 (or g2)
				// might be a subgraph - we want only the number of edges in the subgraph (not edges
				// that extend out of the subgraph)
				M[i][j]=(g1.get_num_edges(i) <= g2.get_num_edges(j) && g1[i].compatible(g2[j]) ? 1 : 0);
				dbgmsg("M[" << i << "][" << j << "] = " << M[i][j]
					<< " g1[" << i << "].size() = " << g1[i].size() 
					<< " g2[" << j << "].size() = " << g2[j].size()
					<< " g1.get_num_edges(" << i << ") = " << g1.get_num_edges(i) 
					<< " g2.get_num_edges(" << j << ") = " << g2.get_num_edges(j)
					<< " compatible = " << boolalpha << g1[i].compatible(g2[j]));
			}
	}
	/*----------------------------------------------------------
	 ...
	 * Returns false if no more pairs are available.
	 -------------------------------------------------------------------------*/
	template<class Graph1, class Graph2>
	bool UllSubState<Graph1, Graph2>::NextPair(node_id &pn1, node_id &pn2, node_id prev_n1, node_id prev_n2) {
	...
	void UllSubState<Graph1, Graph2>::AddPair(node_id node1, node_id node2) {
		...
		for(k=core_len; k<n1; k++)
			M[k][node2]=0;
		dbgmsg("node1 " << node1 << " node2 " << node2 << " core_1.size() " << core_1.size() << " core_2.size() " << core_2.size());
		__refine(); 
	}
	/*--------------------------------------------------------------
	...
	void UllSubState<Graph1, Graph2>::__refine() {
		dbgmsg("n1 " << n1 << " n2 " << n2 << " core_1.size() " << core_1.size() << " core_2.size() " << core_2.size() << " core_len " << core_len);
		for(int i=core_len; i<n1; i++)
			for(int j=0; j<n2; j++) {
				//~ dbgmsg("i = " << i << " j = " << j);
				if (M[i][j]) {
					...
					for(int k=core_len-1; k<core_len; k++) {
						...
						//~ else if (edge_ik  && !g1->CompatibleEdge(g1->GetEdgeAttr(i,k), g2->GetEdgeAttr(j,l))) {
						else if (edge_ik  && !(g1[i].compatible(g2[j]) && g1[k].compatible(g2[l]))) {
							M[i][j]=0;
							break;
						}
						//~ else if (edge_ki && !g1->CompatibleEdge(g1->GetEdgeAttr(k,i), g2->GetEdgeAttr(l,j))) {
						else if (edge_ik  && !(g1[k].compatible(g2[l]) && g1[i].compatible(g2[j]))) {
							M[i][j]=0;
							break;
						}
					}
