...
namespace candock {
namespace graph {
//~ Maxclique::Maxclique (const Array2d<bool> &conn, const vector<double>
//&scores, const float tt) : __conn(conn), __scores(scores), pk(0), level(1),
//Tlimit(tt), V(conn.get_szi()), Q(conn.get_szi()), QMAX(conn.get_szi()) {
Maxclique::Maxclique(const Array2d<bool>& conn, const float tt)
    : __conn(conn),
      pk(0),
      level(1),
      Tlimit(tt),
      ...
      Q(conn.get_szi()) {
    if (conn.get_szi() == 0)
        throw Error("Error: Graph is empty.");  // fixes issue #116
    for (size_t i = 0; i < conn.get_szi(); i++) V.push(i);
    C = new ColorClass[conn.get_szi() + 1];
    for (size_t i = 0; i < conn.get_szi() + 1; i++)
...
//Maxclique::mcqw(const int minsz) {
//~
//~ // search only for cliques of size >= minsz
//~ for (int i = 0; i < minsz - 1; ++i) QMAX.push(0);
//~
    ...
    while (R.size()) {
        if (Q.size() + R.end().get_degree() > QMAX.size()) {
            ...
            if (Rp.size() && Q.size() < QMAX.size() + 1) {
                ...
                //~ cout << "step = " << pk << " current max. clique size = " <<
                //Q.size() << endl;
                //~ QMAXES.push_back(Q);
                vector<unsigned short int> lastQ;
                for (int i = 0; i < Q.size(); ++i) {
