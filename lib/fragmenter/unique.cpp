...
namespace Molib {
	...
	//~ pair<bool, size_t> Unique::__match(MolGraph &g, USeeds::iterator it1, USeeds::iterator it2, size_t &si) {
		...
		for (auto &atom : seed) {
			...
			//~ for (auto &bond : a) {
				...
				if (a.atom_number() < adj_a.atom_number() && seed.find(&adj_a) != seed.end()) {
					...
					//~ vertex1 << a.idatm_type() << "#" << a.atom_number();
					//~ vertex2 << adj_a.idatm_type() << "#" << adj_a.atom_number();
					vertex1 << a.get_label() << "#" << a.atom_number();
					vertex2 << adj_a.get_label() << "#" << adj_a.atom_number();
					//~ s.push_back({vertex1.str(), vertex2.str()});
