...
ostream& operator<<(ostream& os, const AtomMatch& atom_match) {
    for (auto& kv : atom_match) {
        os << "[" << kv.first << "-->" << kv.second->atom_name() << "] ";
    }
    return os;
}
...
Fragmenter::Fragment::Fragment(Atom::Set core, Atom::Set join, Unique& u)
...
void Fragmenter::flip_conjugated_gaff_types(const Atom::Vec& atoms) {
    ...
    for (auto& patom : atoms) {
        if (!visited.count(patom) &&
            (help::gaff_group_1.count(patom->gaff_type()) ||
            ...
            while (!q.empty()) {
                ...
                for (auto& bondee : current) {
                    ...
                    if (!visited.count(&bondee) &&
                        (help::gaff_group_1.count(bondee.gaff_type()) ||
                         help::gaff_group_2.count(bondee.gaff_type()))) {
                        if (bond.is_double() || bond.is_triple() ||
                            bond.is_aromatic()) {
                            if ((help::gaff_group_1.count(
                                     current.gaff_type()) &&
                                 help::gaff_group_1.count(
                                     bondee.gaff_type())) ||
                                (help::gaff_group_2.count(
                                     current.gaff_type()) &&
                                 help::gaff_group_2.count(
                                     bondee.gaff_type()))) {
                                dbgmsg("bondee gaff type (case 1) = "
                                       << bondee.gaff_type());
                                bondee.set_gaff_type(
                                    help::gaff_flip.at(bondee.gaff_type()));
...
void Fragmenter::substitute_bonds(const help::rename_rules& rrules) {
    ...
    for (auto& p : rrules) {
        ...
        for (auto& m : atom_matches) fragmenter.apply_rule(m, p.rule, visited);
    }
}
...
void Fragmenter::substitute_atoms(const help::rename_rules& rrules) {
    ...
    for (auto& p : rrules) {
        AtomMatchVec atom_matches = fragmenter.grep(p.pattern);
        for (auto& m : atom_matches) fragmenter.apply_rule(m, p.rule, visited);
        dbgmsg("RENAME RULE : " << p << endl << "MATCHES : " << atom_matches);
    }
}
    ...
    for (auto& match : matches) {
        ...
        for (bool reverse : {false, true}) {  // two tries : forward and reverse
            try {
                ...
                mvec.push_back(m);
                dbgmsg("in grep : atom match (try " << reverse << ") is "
                                                    << endl
                                                    << m);
            } catch (Error&) {
            }
        ...
        if (visited.empty()) {  // first bond-match considered
            if (!reverse) {
                if (atom11.compatible(atom21)) {
                    ...
                    throw Error("exception : trying with reversed first bond");
            } else {
                if (atom11.compatible(atom22)) {
                    amatch[anum11] = &atom22;
                    amatch[anum12] = &atom21;
                } else
                    throw Error("exception : ran out of options");
            }
        } else {  // not first bond
            if (amatch[anum11] == &atom21 && atom12.compatible(atom22))
                amatch[anum12] = &atom22;
            else if (amatch[anum11] == &atom22 && atom12.compatible(atom21))
                amatch[anum12] = &atom21;
            else if (amatch[anum12] == &atom21 && atom11.compatible(atom22))
                amatch[anum11] = &atom22;
            else if (amatch[anum12] == &atom22 && atom11.compatible(atom21))
                amatch[anum11] = &atom21;
            else
                throw Error(
                    "exception : cannot convert from bond match to atom match");
        }
        visited.insert(&bond1);
        for (auto& bond : bond1)
            if (!visited.count(&bond) && bond_match.count(&bond)) q.push(&bond);
    }
    return amatch;
}

Fragmenter::Fragment::Vec Fragmenter::identify_overlapping_rigid_segments(
    const Atom::Vec& atoms, Unique& u) {
