...
ostream& operator<<(ostream& stream, const ForceField::ResidueTopology& r) {
    ...
    for (auto& kv1 : r.bond) {
        for (auto& kv2 : kv1.second) {
            stream << "bond " << kv1.first << " " << kv2.first << " = "
                   << kv2.second << endl;
        }
    }
...
const ForceField::KBType& ForceField::get_kb_force_type(
    const molib::Atom& atom1, const molib::Atom& atom2) const {
...
#endif
    if (kb_force_type.count(aclass1) != 0 &&
        kb_force_type.at(aclass1).count(aclass2) != 0) {
        return kb_force_type.at(aclass1).at(aclass2);
    }

    if (kb_force_type.count(aclass2) != 0 &&
        kb_force_type.at(aclass2).count(aclass1) != 0) {
...
const ForceField::KBType& ForceField::get_kb_force_type(int aclass1,
                                                        int aclass2) const {
    if (kb_force_type.count(aclass1) != 0 &&
        kb_force_type.at(aclass1).count(aclass2) != 0) {
        return kb_force_type.at(aclass1).at(aclass2);
    ...
    stringstream ss;
    ss << "warning : missing kb force type " << help::idatm_unmask[aclass1]
       << "-" << help::idatm_unmask[aclass2] << " requested " << aclass1 << " "
       << aclass2;
    throw ParameterError(ss.str());
}
...
const ForceField::AtomType& ForceField::get_atom_type(const int type) const {
    if (has_atom_type(type) != 0) return atom_type.at(type);
    throw ParameterError("warning : missing atom type " + std::to_string(type));
}

bool ForceField::has_bond_type(const string& aclass1,
                               const string& aclass2) const {
...
bool ForceField::has_angle_type(const string& aclass1, const string& aclass2,
                                const string& aclass3) const {
...
const ForceField::AngleType& ForceField::get_angle_type(const int type1,
    ...
    if (has_angle_type(aclass1, aclass2, aclass3))
        return angle_type.at(aclass1).at(aclass2).at(aclass3);
    if (has_angle_type(aclass3, aclass2, aclass1))
...
bool ForceField::has_dihedral_type(const string& aclass1, const string& aclass2,
                                   const string& aclass3,
                                   const string& aclass4) const {
    return (torsion_type.count(aclass1) != 0 &&
            ...
            torsion_type.at(aclass1).at(aclass2).at(aclass3).count(aclass4) !=
                0);
}

const ForceField::TorsionTypeVec& ForceField::get_dihedral_type(
    const int type1, const int type2, const int type3, const int type4) const {
    const AtomType& atype1 = atom_type.at(type1);
    const AtomType& atype2 = atom_type.at(type2);
    const AtomType& atype3 = atom_type.at(type3);
    ...
    dbgmsg("add dihedral type1 = "
           << type1 << " type2 = " << type2 << " type3 = " << type3
           << " type4 = " << type4 << " aclass1 = " << aclass1 << " aclass2 = "
           << aclass2 << " aclass3 = " << aclass3 << " aclass4 = " << aclass4);
    ...
    for (auto& sclass :
         help::get_replacement({aclass1, aclass2, aclass3, aclass4})) {
...
bool ForceField::has_improper_type(const string& aclass1, const string& aclass2,
    ...
    return (improper_type.count(aclass1) != 0 &&
            ...
            improper_type.at(aclass1).at(aclass2).at(aclass3).count(aclass4) !=
                0);
}

const ForceField::TorsionTypeVec& ForceField::get_improper_type(
    const int type1, const int type2, const int type3, const int type4) const {
    const AtomType& atype1 = atom_type.at(type1);
    const AtomType& atype2 = atom_type.at(type2);
    const AtomType& atype3 = atom_type.at(type3);
...
ForceField& ForceField::insert_topology(const molib::Molecule& molecule) {
    ...
    for (auto& presidue : molecule.get_residues()) {
        ...
        // additionally check if residue not already in the ffield (this is for
        // adding receptor
        // topology where amino acids are already in the ffield, but cofactors
        // are not
        if (!this->residue_exists(residue.resn())) {
            if (help::ions.count(residue.resn()) != 0) {
                this->residue_topology.insert(
                    {residue.resn(), ResidueTopology()});
                ResidueTopology& rtop =
                    this->residue_topology.at(residue.resn());
            ...
            for (auto& atom : residue) {
                if (!atom_name_to_type.count(
                        atom.gaff_type())) {  // see issue #113
                    throw Error(
                        "die : insert topology failed for atom with gaff type "
                        "(check gaff.dat) = " +
                        atom.gaff_type());
                }
                rtop.atom.insert(
            ...
            dbgmsg("inserted topology for residue " << residue.resn() << endl
                                                    << rtop);
        }
    }
    return *this;
...
ForceField& ForceField::erase_topology(const molib::Molecule& molecule) {
    for (auto& presidue : molecule.get_residues()) {
        auto& residue = *presidue;
        dbgmsg("erasing topology for residue "
               << residue.resn() << endl
               << residue_topology.at(residue.resn()));
...
ForceField& ForceField::parse_gaff_dat_file(const string& gaff_dat_file) {
    ...
    int semaphore = 0;
    int type = 10000 + gaff_name_to_type.size();  // start ligand types with
                                                  // 10000 to NOT overlap with
                                                  // receptor
    for (auto& line : gdf) {
        ...
        if (semaphore == 1 &&
            std::regex_search(
                line, m,
                std::regex("^(\\S{1,2})\\s*-(\\S{1,2})\\s+(\\S+)\\s+(\\S+)"))) {
        ...
        if (semaphore == 2 &&
            ...
            if (m[1].matched && m[2].matched && m[3].matched && m[4].matched &&
                ...
                const double angle =
                    stod(m[5].str()) * OpenMM::RadiansPerDegree;
        ...
        if (semaphore == 3 &&
            std::regex_search(
                line, m,
                std::regex("^(\\S{1,2})\\s*-(\\S{1,2})\\s*-(\\S{1,2})\\s*-(\\S{"
                           "1,2})\\s+(\\S+)\\s+(\\S+)\\s+(\\S+)\\s+(\\S+)"))) {
            if (m[1].matched && m[2].matched && m[3].matched && m[4].matched &&
                ...
                const double k =
                    (1 / idivf) * stod(m[6].str()) * OpenMM::KJPerKcal;
                const double phase =
                    stod(m[7].str()) * OpenMM::RadiansPerDegree;
                const int periodicity = abs(stoi(m[8].str()));
                TorsionTypeVec& dihedral =
                    this->torsion_type[cl1][cl2][cl3][cl4];
                dihedral.push_back(TorsionType{periodicity, phase, k});
                dbgmsg("periodic (proper) torsion force between "
                       << cl1 << " and " << cl2 << " and " << cl3 << " and "
                       << cl4 << " is "
                       << " periodicity" << dihedral.size() << " = "
                       << periodicity << " phase" << dihedral.size() << " = "
                       << phase << " k" << dihedral.size() << " = " << k);
            }
        }
        ...
        if (semaphore == 4 &&
            ...
            if (m[1].matched && m[2].matched && m[3].matched && m[4].matched &&
                ...
                const double phase =
                    stod(m[6].str()) * OpenMM::RadiansPerDegree;
                const int periodicity = abs(stoi(m[7].str()));
                TorsionTypeVec& improper =
                    this->improper_type[cl1][cl2][cl3][cl4];
                improper.push_back(TorsionType{periodicity, phase, k});
                dbgmsg(
                    "periodic (improper) torsion force between (third atom is "
                    "central) "
        ...
        if (non_bonded &&
            std::regex_search(
                line, m, std::regex("^\\s{2}(\\S{1,2})\\s+(\\S+)\\s+(\\S+)"))) {
            if (m[1].matched && m[2].matched && m[3].matched) {
                ...
                dbgmsg("nonbonded force for atom type "
                       << type << " charge = " << at.charge << " sigma = "
                       << at.sigma << " epsilon = " << at.epsilon);
            }
        }
    }
...
void ForceField::output_forcefield_file(
    const string& fn) {  // output XML forcefield & topology file
    ...
    for (auto& kv : this->atom_type) {
        ...
        type_node->append_attribute(
            doc.allocate_attribute("name", str(doc, name)));
        type_node->append_attribute(
            doc.allocate_attribute("class", at.cl.c_str()));
        type_node->append_attribute(
    ...
    for (auto& kv : this->residue_topology) {
        ...
        for (auto& kv2 : rtop.bond) {
            ...
            for (auto& kv3 : kv2.second) {
                ...
                bond_node->append_attribute(
                    doc.allocate_attribute("to", str(doc, to)));
            }
        }
    }
    ...
    for (auto& kv : this->bond_type) {
        ...
        for (auto& kv2 : kv.second) {
            ...
            bond_node->append_attribute(
                doc.allocate_attribute("k", str(doc, bt.k)));
        }
    }
    ...
    for (auto& kv : this->angle_type) {
        ...
        for (auto& kv2 : kv.second) {
            ...
            for (auto& kv3 : kv2.second) {
                ...
                xml_node<>* angle_node =
                    doc.allocate_node(node_element, "Angle");
    ...
    xml_node<>* pt_node =
        doc.allocate_node(node_element, "PeriodicTorsionForce");
            ...
            for (auto& kv3 : kv2.second) {
                ...
                for (auto& kv4 : kv3.second) {
                    ...
                    xml_node<>* torsion_node =
                        doc.allocate_node(node_element, "Proper");
                    ...
                    for (auto& tt : kv4.second) {
                        ...
                        torsion_node->append_attribute(doc.allocate_attribute(
                            str(doc, ("periodicity" + num)),
                            str(doc, tt.periodicity)));
                        torsion_node->append_attribute(doc.allocate_attribute(
                            str(doc, "phase" + num), str(doc, tt.phase)));
                        torsion_node->append_attribute(doc.allocate_attribute(
                            str(doc, "k" + num), str(doc, tt.k)));
                    }
                }
            }
        }
        ...
        for (auto& kv2 : kv.second) {
            ...
            for (auto& kv3 : kv2.second) {
                ...
                for (auto& kv4 : kv3.second) {
                    ...
                    pt_node->append_node(torsion_node);
                    // be careful because in xml file FIRST atom is central and
                    // in gaff dat type
    ...
    xml_node<>* nb_force_node =
        doc.allocate_node(node_element, "NonbondedForce");
    ff_node->append_node(nb_force_node);
    nb_force_node->append_attribute(doc.allocate_attribute(
        "coulomb14scale", str(doc, this->coulomb14scale)));
    ...
    for (auto& kv : this->kb_force_type) {
        ...
        for (auto& kv2 : kv.second) {
            ...
            kb_bond_node->append_attribute(
                doc.allocate_attribute("class1", help::idatm_unmask[cl1]));
            kb_bond_node->append_attribute(
                doc.allocate_attribute("class2", help::idatm_unmask[cl2]));
            stringstream ss;
            copy(kbt.potential.begin(), kbt.potential.end(),
...
ForceField& ForceField::parse_forcefield_file(
    const string& fn) {  // read XML forcefield & topology file
    using namespace rapidxml;
    string ff_file;
    ...
    for (xml_node<>* node =
        ...
        for (xml_node<>* node2 = node->first_node("Atom"); node2;
            ...
            dbgmsg("parsing residue topology atom name = "
                   << node2->first_attribute("name")->value() << " type = "
        ...
        for (xml_node<>* node2 = node->first_node("Bond"); node2;
            ...
            const string atom_name1 =
                names[stoi(node2->first_attribute("from")->value())];
            const string atom_name2 =
    ...
    if (torsions_node) {
        for (xml_node<>* node = torsions_node->first_node(); node;
            ...
            const bool is_proper =
                (string("Proper").compare(node->name()) == 0);
            dbgmsg("node_name = " << node->name()
                                  << " is_proper = " << is_proper);
            ...
            while (attr) {
                ...
                if (is_proper) {
                    ...
                    dbgmsg("periodic torsion (dihedral) force between "
                           ...
                           << " periodicity" << dihedral.size() << " = "
                           << periodicity << " phase" << dihedral.size()
                           << " = " << phase << " k" << dihedral.size() << " = "
                           << k);
                } else {
                    TorsionTypeVec& improper =
                        this->improper_type[cl2][cl3][cl1][cl4];
                    improper.push_back(TorsionType{periodicity, phase, k});
                    dbgmsg("periodic torsion (improper) force between "
                           << cl1 << " and " << cl2 << " and " << cl3 << " and "
                           << cl4 << " is "
                           << " periodicity" << improper.size() << " = "
                           << periodicity << " phase" << improper.size()
                           << " = " << phase << " k" << improper.size() << " = "
                           << k << " (here the FIRST atom is central)");
                }
            }
        }
    }
    ...
    if (nb_node) {
        ...
        for (xml_node<>* node = nb_node->first_node(); node;
            ...
            dbgmsg("nonbonded force for atom type "
                   << type << " charge = " << at.charge
                   << " sigma = " << at.sigma << " epsilon = " << at.epsilon);
        }
    }
    ...
    if (kb_node) {
        ...
        for (xml_node<>* node = kb_node->first_node(); node;
             node = node->next_sibling()) {
            const int cl1 =
                help::idatm_mask.at(node->first_attribute("class1")->value());
            const int cl2 =
                help::idatm_mask.at(node->first_attribute("class2")->value());
            vector<string> pot =
                help::ssplit(node->first_attribute("potential")->value());
            vector<string> der =
                help::ssplit(node->first_attribute("derivative")->value());
