...
vector<pair<ChResi, int> > Denorm::get_cons(int cluster_id) {
    ...
    */
    ...
    while (get_next_residue(r1, r2)) {
        ret.push_back(make_pair(ChResi(r1->chain_id, r1->resi),
                                1));  // 1 je oznaka za prvi protein
        ret.push_back(make_pair(ChResi(r2->chain_id, r2->resi),
                                2));  // 2 je oznaka za drugi protein
    }
    return ret;
}

void Denorm::free(int cluster_id) {
    cout << "Deleting denorm internal structures, except for U and t of "
            "cluster_id = "
         << cluster_id << " ..." << endl;
    for (map<int, ClusterData*>::iterator it = cluster.begin();
         it != cluster.end(); it++) {
        if (it->first != cluster_id) delete it->second;
    }
    for (multimap<int, pair<Residue*, Residue*> >::iterator it =
             aligned.begin();
         it != aligned.end(); it++) {
...
bool Denorm::read_one(string& row) {
    /*
      Preberemo samo pdb_id in chain_id v denormalizirani vrstici in podatke
      zapisemo v interne strukture.
    */
    vector<string> all = reg_split(row, "\t");
    if (all.size() != 4) {
        cout << "Warning (DENORM) : Wrong format of this nosql line:" << row
             << endl;
        return false;
    }
    ...
    string chain_id2 =
        (all[1].at(0) == '*' ? all[1].substr(1) : all[1]);  // ce je inverted
    this->pdb_id = pdb_id2;
...
bool Denorm::read(string& row, bool filter) {
    /*
        Preberemo denormalizirano vrstico in podatke zapisemo v interne
       strukture. Hkrati tudi filtriramo
    ...
    bool swap =
        (all[1].at(0) == '*'
             ? true
             : false);  // ce je zvezdica, sta protein1 in protein2 zamenjana
    string chain_id2 =
        (all[1].at(0) == '*' ? all[1].substr(1) : all[1]);  // ce je inverted
    ...
    for (vector<string>::iterator it = all_clusters.begin();
         it != all_clusters.end(); it++) {
        ...
        //    if (!filter || (blosum_score < BLOSUM_SCORE && calpha_rmsd <
        //    CALPHA_RMSD && surf_vector_angle < SURF_VECTOR_ANGLE && scons >
        //    SCONS)) {
        if (!filter || (z_score(cluster_score) > Z_SCORE)) {
            ...
            // ... ce sta proteina zamenjana, potem invertiramo matriko in
            // negiramo vektor
            if (swap) {
                Kabsch k;
                k.invert(U, t);
            ...
            // pointer na alignan protein (Molecule*) dodamo kasneje (glej
            // NoSql::read)
            cluster[cluster_id] =
                new ClusterData(cluster_id, scons, blosum_score, calpha_rmsd,
                                surf_vector_angle, cluster_score, NULL, U, t);
        }
    }
    ...
    // preberemo results
    set<ChResi> rid;
    // drugacen DB query, zato je vedno treba preverjati, po katerem proteinu
    // smo dejansko iskali
    vector<string> all_results = reg_split(results, "),(", "(", ")");
    for (vector<string>::iterator it = all_results.begin();
        ...
        if (cluster.find(cluster_id) != cluster.end()) {
            ...
            aligned.insert(make_pair(
                cluster_id,
                make_pair(new Residue(ch1, rei1),
                          new Residue(ch2, ren2, rei2, cluster[cluster_id],
                                      NULL, (flx == 'Y') ? true : false))));

            /* ker uporabljamo multimap, v katerem se klastri lahko prekrivajo,
             * moramo dolociti dolzino alignmenta posebej */
