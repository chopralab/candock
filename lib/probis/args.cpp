...
void Args::read_constants(string par_file) {
    ...
    if (!par.is_open()) {
        throw Err("Error (ARGS) : Cannot open the parameter file " + par_file,
                  14);
    }
...
Args::Args() {
    opts.insert(make_pair(
        "-align",
        "Read a rotational matrix of an alignment from an .nosql file and "
        "superimpose the two given proteins accordingly (first run -compare or "
        "-surfdb). Output the superimposed proteins' coordinates in a .pdb "
        "file. You need to provide both .pdb files that you want to "
        "superimpose (see -f1, -c1, -f2, c2 modifiers) and an alignment number "
        "(see -alno modifier)."));
    opts.insert(make_pair("-compare",
                          "Compare two protein surfaces (.pdb or .srf files). "
                          "(If you use .pdb files, surfaces will be computed "
                          "first.) Output their local structural alignments in "
                          "an .nosql file. Each alignment consists of a "
                          "rotational matrix, alignment scores, and aligned "
                          "residues of the compared proteins."));
    opts.insert(make_pair(
        "-extract",
        "Calculate surface of a protein. Redirect the output (which is the "
        "surface) to a surface (.srf) file. Surface files can be used instead "
        "of .pdb files together with -compare or -surfdb options, which "
        "improves performance when doing repetitive comparisons (because "
        "surface does not need to be recalculated for each comparison). Option "
        "-surfdb works with .srf files exclusively!"));
    opts.insert(make_pair(
        "-results",
        "Read alignments from an .nosql file, filter them according to their "
        "scores, and calculate fingerprint residues (which can also be used as "
        "filter). Output results in Json format. In addition, replace "
        "B-factors in the query protein’s PDB file with the degrees of "
        "structural conservation. If used with -ligdir modifier, output "
        "ligands in Json format as well."));
    opts.insert(make_pair(
        "-surfdb",
        "Compare the query protein surface (.srf) with other protein surfaces "
        "listed in the SURF_FILE (see -sfile modifier). This does the same "
        "calculation as the -compare option, but faster, because protein "
        "surfaces are precalculated (see -extract option). This options also "
        "supports parallel computation on multiple CPUs. Output is the same as "
        "with -compare."));
    opts.insert(make_pair("-h",
                          ...
                          "modifier)."));
    mods.insert(make_pair("-alno ALIGNMENT_NO",
                          "Each comparison of a pair of proteins may result in "
                          "many different local structural alignments.The "
                          "alignment number can be 0 to 4 (see default "
                          "CLUS_TO_OUTPUT parameter)."));
    mods.insert(
        make_pair("-ncpu NCPU", "Use this number of concurrent threads."));
    mods.insert(make_pair(
        "-bsite BSITE, -bsite1 BSITE, or -bsite2 BSITE",
        "This selects protein residues in a certain radius (set by -dist) "
        "around the given ligand, and takes these residues as input (use with "
        "-extract or -compare options). If used with -compare, it only works "
        "with .pdb files (not .srf).For example: ‘-bsite ATP.305.A’ - ATP "
        "(residue name), 305 (residue number), A (chain id) or ‘-bsite *.*.B’ "
        "- chain B is the ligand (so you can select protein-protein binding "
        "sites as input)"));
    mods.insert(make_pair("-c1, c2",
                          "Chain identifiers of the compared proteins. You may "
                          "give multiple chains, e.g., '-c1 ABC'."));
    mods.insert(make_pair("-database",
                          "Used by the web server (with -surfdb and -compare "
                          "options). It will output an .nosql file as usual, "
                          "and additional .nosql file with inversed rotational "
                          "matrices, whose lines are marked with asterisks."));
    mods.insert(make_pair("-dist INTER_CHAIN_DIST",
                          "The distance between protein chains or between "
                          "ligand and protein. Use with -bsite modifier or "
                          "-mark and -results options. "));
    mods.insert(
        make_pair("-srffile", "Surface file (.srf) for input or output."));
    mods.insert(
        make_pair("-f1, f2", "Compared proteins' files (.pdb or .srf)."));
    mods.insert(make_pair("-in INDIR", "Directory where input files are."));
    mods.insert(make_pair(
        "-local",
        "Use this to perform local alignments search (with -compare or -surfdb "
        "options). By default, after the local alignment is found (with "
        "maximum clique algorithm), an attempt is made to extend this "
        "alignment along the backbones of the compared proteins. In this way, "
        "parts of proteins that adopt different conformations (e.g., loops) "
        "can be aligned (these aligned residues are marked with 'flx' in "
        "alignments.json)."));
    mods.insert(make_pair("-longnames",
    ...
    mods.insert(make_pair(
        ...
        "protein structure (use with -extract or -compare options). This will "
        "generate a .srf file with only the selected residues. To select some "
        "residues on chains A and B of the input protein, use -motif \"[:A and "
        "(14,57,69-71) or :B and (33,34,50)]\". Note that chain Iids are case "
        "sensitive. Square brackets are mandatory!"));
    mods.insert(make_pair(
        "-nobb",
        "Do not include descriptors originating from backbone atoms."));
    mods.insert(make_pair("-nomarkbb",
                          "Turns off the default action, which is to mark (not "
                          "delete) backbone descriptors. In the first step of "
                          "filtering, only non-backbone descriptors are used, "
                          "while in the maximum clique step, all descriptors "
                          "are used."));
    mods.insert(make_pair("-noclus",
                          "Local structural alignments found (maximum cliques) "
                          "are not clustered."));
    mods.insert(make_pair("-nofp",
                          "Do not calculate fingerprint residues. Do not "
                          "filter by fingerprint residues (use with -results "
                          "option)."));
    mods.insert(make_pair("-noprune",
    ...
    mods.insert(
        make_pair("-out OUTDIR", "Directory to write output files to."));
    mods.insert(
        make_pair("-param PAR_FILE",
                  "Read parameters from the specified parameter file."));
    mods.insert(make_pair(
        "-sfile SURF_FILE",
        "Specify file that contains names of .srf files to be compared with "
        "the query protein (see -surfdb option). Each line must contain one "
        ".srf file-name.Example:{newline}protein1.srf A{newline}protein2.srf "
        "B{newline}protein3.srf A{newline}etc."));
    mods.insert(make_pair(
        "-super",
        "Find local structural alignments between two proteins (use with "
        "-compare option) and superimpose the two proteins according all "
        "alignments found.  For each alignment, output the '.rota.pdb' file "
        "with the proteins superimposed according to this alignment."));
    mods.insert(make_pair(
        "-verbose",
        "Output debugging information. Use when testing the program."));
    mods.insert(make_pair("-z_score Z_SCORE",
                          "The cutoff value for z_score. Low z_score (<2) "
                          "means that more insignificant alignments will be "
                          "outputted (these can also occur by chance), higher "
                          "z_score (>2) means only significant alignments will "
                          "be outputted (use with -results option)."));
    mods.insert(make_pair("-ligdir DIRNAMES",
                          "Directory where the names of proteins as files: "
                          "each file is PdbIdChainId, e.g., 1allA."));
    mods.insert(
        make_pair("-nosql NOSQL_FILE",
                  "Output / read nosql alignments to / from this file."));
    mods.insert(
        make_pair("-json JSON_FILE", "Output json alignments to this file."));
}

ostream& operator<<(ostream& out, Args& arg) {
    //
    // Kopiraj iz google docs v opts.txt in potem: cat opts.txt|sed -r
    // 's/^-(.*)/"));\nopts.insert(make_pair("-\1",/'|sed 's/^$/"/'
    // Izbrisi vse \n , nadomesti z {newline} !
    //
    cout << "Protein Binding Sites by Local Structural Alignments --- Ver. "
            "2.4.4 --- Sep 11 2014 ---"
         << endl;
    cout << "Usage: probis {OPTION} [MODIFIERS] -f1 PDB_FILE -c1 CHAIN_ID [-f2 "
            "PDB_FILE -c2 CHAIN_ID]"
         << endl;
    unsigned int n = thread::hardware_concurrency();
    cout << endl
         << "Detected support for " << n << " concurrent threads." << endl;
    ...
    out << endl
        ...
        << endl
        << endl;
    for (map<string, string>::iterator it = arg.opts.begin();
         it != arg.opts.end(); it++) {
        vector<string> s;
        size_t i = 0, i_new = 0;
        while ((i_new = it->second.find_first_of(" \n", i + 60)) !=
               string::npos) {
            out << setw(47) << (i == 0 ? it->first : "") << setw(10) << ""
                << it->second.substr(i, i_new - i) << endl;
            i = i_new;
        }
        out << setw(47) << (i == 0 ? it->first : "") << setw(10) << ""
            << it->second.substr(i, it->second.size()) << endl
            << endl;
    }
    out << endl
        << "---------------------------------------- MODIFIERS "
        ...
        << endl
        << endl;
    for (map<string, string>::iterator it = arg.mods.begin();
         it != arg.mods.end(); it++) {
        vector<string> s;
        size_t i = 0, i_new = 0;
        while ((i_new = it->second.find_first_of(" \n", i + 60)) !=
...
void Args::fill_args(int argc, char* argv[]) {
    ...
    if (strstr(str, "-compare")) {
        ...
        _state = MARK;
    } else if (strstr(str, "-lig ")) {  // <- presledek mora biti za "lig"
                                        // drugace je zmesnjava z "ligdir"
        _state = LIGAND;
    ...
    if (PROTEIN1.find("srf") != string::npos ||
        PROTEIN1.find("SRF") != string::npos)
        srf1 = true;
    if (PROTEIN2.find("srf") != string::npos ||
        PROTEIN2.find("SRF") != string::npos)
        srf2 = true;
    if (!PROTEIN2.empty() && srf1 != srf2)
        throw Err(
            "Error (ARGS) : Both proteins should be in the same format (.srf "
            "or .pdb) !",
            11);
    else if (srf1)
        _srf = true;
...
void Args::print_state() {
    switch (_state) {
        case TOTAL:
            cout << "--- Complete proteins' search ---" << endl;
            break;
        //  case PPI    : cout << "--- Protein-protein interaction search ---"
        //  << endl; break;
        //  case BIND   : cout << "--- Binding cavity search ---" << endl;
        //  break;
        case WRITE:
            cout << "--- Writing out surface of a protein ---" << endl;
            break;
        case SURFDB:
            cout << "--- Database search for a complete protein ---" << endl;
            break;  // obsolete
        case MARK:
            cout << "--- Find interface residues ---" << endl;
            break;
        case LIGAND:
            cout << "--- Find existing binding sites & ligands ---" << endl;
            break;
        case RESULTS:
            cout << "--- Reading results from nosql and outputing alignments "
                    "as Json ---"
                 << endl;
            break;
        //  case SEQ    : cout << "--- Reading sequence ---" << endl; break;
        case ALIGN:
            cout << "--- Aligning the protein structure ---" << endl;
            break;
        //  case BIOU   : cout << "--- Generating biological units ---" << endl;
        //  break;
        case HEAD:
            cout << "--- Ligand names are output ---" << endl;
            break;
        case ALLBIO:
            cout << "--- Output all biological units for a given PDB file ---"
                 << endl;
            break;
#ifdef CILE
        case PATCH:
            cout << "--- Generate surface patches ---" << endl;
            break;
#endif
            //  case SUPER  : cout << "--- Superimposing two protein structures
            //  ---" << endl; break;
    }
}
