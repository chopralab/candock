...
void Probe::output(int lcolor, bool _motif) {
  /*
    Izpisemo probe centre. Ce je _motif == true, potem izpisemo samo tiste, ki pripadajo najvecjemu surface-u.
    Ce na primer izberemo kot motif dva patcha na povrsini, ki nista skupaj, potem bi v nasprotnem primeru imeli
...
void Probe::pdb() {
  char line[SMALL];
  sprintf(line,"HETATM%5d%4s%5s%2s%4d%12.3f%8.3f%8.3f%6s%6.2f", 1, "CX", "XXX", "X", this->num, this->crd.x, this->crd.y, this->crd.z, "1.00", this->dist);
  cout << line << endl;
}
...
void Probe::output_probe() {
  /*
    Output probe centers and color them according to which surface they belong to.
  */
