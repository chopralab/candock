...
void Clique::assign_connect(Item *item) {
  /*
    Assign edges of i-th subgraph to the connect table e[][]; when an edge is present between 
...
bool by_degree (Vert *vi, Vert *vj) { return (vi->size_edge > vj->size_edge); }  // callback for sorting vertices by degree
...
//      A.push_back( B[j] ); // (vi, color, edges, row)
      A.push_back(new Vert(B[j]->lista_id, B[j]->cluster_id, B[j]->vertex, 0, B[j]->size_edge, B[j]->row) ); // (vi, color, edges, row)
...
void Clique::COLOR_SORT(vector<Vert* > &R) {
  ...
  maxno = 1;
  min_k = QMAX->vert.size() - Q->vert.size() ; // change to + 1 when calculating only one maximum clique !!
  ...
  C.push_back(vec);
  C[1].reserve( R.size() );
  C[2].reserve( R.size() ); // for sure reserve a positive number (not R.vert.size() - 1 )
...
void Clique::COPY(vector<Vert* > &A, vector<Vert* > &B) {
  /*
    Deallocate each vertex of A and clear the vector array, then refill it with copies of vertices of B
  */
  ...
  for (unsigned int i = 0; i < B.size(); i++)
    A.push_back(new Vert(B[i]->lista_id, B[i]->cluster_id, B[i]->vertex, B[i]->color, B[i]->size_edge, B[i]->row ) );
}
...
//#endif
//  int w = 0;
//  for (int i = 0; i < (signed) A->vert.size() - 1; i++) {   // signed here is crucial otherwise expression is INF
//    for (unsigned int j = i; j < A->vert.size(); j++)
////      w += lookup_weight[pair<int, int > (make_pair(A->vert[i]->vertex, A->vert[j]->vertex ) ) ];
//      w += A->weight_edge[pair<int, int > (make_pair(A->vert[i]->vertex, A->vert[j]->vertex ) ) ];
//  }
//#ifndef NDEBUG
...
//  CNT.vert[level]->vertex = CNT.vert[level]->vertex + CNT.vert[level - 1]->vertex - CNT.vert[level]->color;
//  CNT.vert[level]->color = CNT.vert[level - 1]->vertex;

  while (R.size() != 0) {
    p = R.back()->vertex;  // p is assigned the last vertex in R
//    p = R.vert[R.vert.size() - 1]->vertex;  // p is assigned the last vertex in R

    if (Q->vert.size() + R.back()->color >= QMAX->vert.size()) {
//    if (Q->vert.size() + R.back()->color > QMAX->vert.size() && Q->vert.size() < 5) {
      
//    if (Q->vert.size() + R.vert[R.vert.size() - 1]->color > QMAX->vert.size() ) {
      
//      Q->vert.push_back( R.back() );
      Q->vert.push_back(new Vert(R.back()->lista_id, R.back()->cluster_id, p, 0, 0, R.back()->row ) );
...
//      if ((tmp = superimpose(Q, Q.U, Q.t)) < RMSD_INCR + 1000.0) {  // ocitno je preverjanje rmsd-jev izklopljeno!
//
//        Q.rmsd = tmp;
...
//            //      if (item->edge[k].second == item->vert[j]->vertex )
...
//          for (unsigned int k = 0; k < Rp.edge.size(); k++) {
...
//        CNT.vert[level]->vertex++;
        ...
        if (!(pk % 100)) 
          if ((double)(clock() - start) / CLOCKS_PER_SEC > CLIQUE_TLIMIT ) {
            dbgmsg("Clique::EXPAND Time limit breached after " << (double)(clock() - start) / CLOCKS_PER_SEC << "s");
...
//        /* if a larger clique was found than one in QMAX delete all QMAX */
...
//          for (unsigned int i = 0; i < Q->vert.size(); i++)
...
//          }
//          else
//            if ( superimpose(*Q, Q->U, Q->t) < superimpose(*QMAX, QMAX->U, QMAX->t) ) {
...
#endif
            //              dbgmsg("Q->rmsd = " << superimpose(*Q, Q->U, Q->t));
            //              dbgmsg("QMAX->rmsd = " << superimpose(*QMAX, QMAX->U, QMAX->t));
            COPY(QMAX->vert, Q->vert);
          }
        }
        else {
...
#ifndef NDEBUG
        dbgmsg("EXPAND: Q->vert.size() = " << Q->vert.size() << " QMAX->vert.size() = " << QMAX->vert.size());
#endif
        
        
#ifndef NDEBUG
        dbgmsg("found maximal clique QMAX->vert.size() = " << QMAX->vert.size());
...
//            Q.size--;
//#ifndef NDEBUG
//            dbgmsg("morebitna napaka Q.rmsd = " << Q.rmsd << " tmp = " << tmp << " super = " << superimpose(Q, Q.U, Q.t));
//#endif
//            Q.size++;
...
//            Q.size--;
//#ifndef NDEBUG
//            dbgmsg("morebitna napaka 2 Q.rmsd = " << Q.rmsd << " tmp = " << tmp << " super = " << superimpose(Q, Q.U, Q.t));
//#endif
//            Q.size++;
//            gsl_matrix_memcpy(Q.U, Q.U_old);
//            gsl_vector_memcpy(Q.t, Q.t_old);
...
#ifndef NDEBUG
    dbgmsg("CLIQUE> Finished "<<i+1<<"/"<<subgraph->item.size()<<" cliques [" << (double)(clock() - start) / CLOCKS_PER_SEC << "s]");
#endif
  }

  dbgmsg("Clique::max_clique() Time = " << (double)(clock() - lokalni_start) / CLOCKS_PER_SEC);
...
//  subgraph->item = (Item*) calloc(num_i + 1, sizeof(Item));
  ...
  while (!f.eof() ) {
    ...
    if (buffer[0] == 'p') {
      ...
      subgraph->item[num_i]->edge.reserve(3 * size_vertex); // we reserve a bit more space for edges (but not exactly)

//      subgraph->item[num_i].vert = (Vert*) calloc(subgraph->item[num_i].size, sizeof(Vert));
...
//        subgraph->item[num_i].vert[i].item.vert = (Vert*)  calloc(subgraph->item[num_i].size, sizeof(Vert));
      }
    }
    if (buffer[0] == 'e') {
      i = 2;
...
//      subgraph->item[num_i].vert[vi].item.vert[subgraph->item[num_i].vert[vi].item.size++].vertex = vj;
//      subgraph->item[num_i].vert[vj].item.vert[subgraph->item[num_i].vert[vj].item.size++].vertex = vi;
