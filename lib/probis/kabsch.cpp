...
Descriptor* Kabsch::align_descriptors(Descriptor* desc1, Descriptor* desc2,
                                      Item* qmax) {
    /*
      Return the pointer to the two aligned sets of descriptors.
      Input: two descriptors sets and a maximum clique with the rotational and
    ...
    */
    ...
    for (Descriptor* d1 = desc1; d1 != NULL; d1 = d1->next) {
        ...
        //    cout << "mnsp = " << tmp->s->mnsp << "x = " << tmp->crd.x << " y =
        //    " << tmp->crd.y << " z = " << tmp->crd.y << endl;
        tmp->psurf = 0;
        tmp->ref = d1;
        tmp->next = dtot;
        dtot = tmp;
...
static inline void gsl_vector_cross(const gsl_vector* a, const gsl_vector* b,
                                    gsl_vector* c) {
...
int Kabsch::kabsch(unsigned int size, /* the number of points */
    ...
    if (size == 1) {
        ...
        if (gsl_vector_get(eval, 1) > NORM_EPS) {
            ...
            gsl_vector_cross(&a0.vector, &a1.vector,
            ...
            if (norm_b0 > NORM_EPS && norm_b1 > NORM_EPS) {
                ...
                gsl_vector_scale(&b1.vector,
                                 1.0 / norm_b1); /* b1 = ||R * a1|| */
                gsl_vector_cross(&b0.vector, &b1.vector,
...
double Kabsch::superimpose_desc(Item* item) {
    /*
      Prilegaj deskriptorje, vendar vrni rmsd med C-alpha atomi. Na ta nacin
      preprecis
      primere, ko sta prilegani funkcionalni skupini obrnjeni ena proti drugi.

      Upostevamo vec istih calpha atomov.

      Tukaj dolocimo tudi item->U in item->t, saj ROTA kaze na item->U in trans
      na item->t.
    */
    gsl_matrix* ROTA = item->U;
    gsl_vector* trans = item->t;
    ...
    /* ce prileganje uspe, potem superimposamo, po dobljenih matrikah, calpha
     * atome, in izracunamo rmsd med calpha */
    if (kabsch(item->vert.size(), X, Y, ROTA, trans, NULL) == 1) {
        double rmsd = 0.0;

        for (unsigned int i = 0; i < item->vert.size(); i++) {
            Coor c = rotate_vector(
                item->vert[i]->row->desc1->atom->acid_start->next->crd, ROTA,
                trans);

            rmsd += dist_fast(
                c, item->vert[i]->row->desc2->atom->acid_start->next->crd);
        }
    ...
    */
    ...
    if (kabsch(item->vert.size(), X, Y, ROTA, trans, NULL) == 1) {
        ...
        for (unsigned int i = 0; i < item->vert.size(); i++) {
            ...
            if (unique_resi.find(make_pair(resi1, resi2)) ==
                unique_resi.end()) {
                unique_resi.insert(make_pair(resi1, resi2));

                Coor c = rotate_vector(
                    item->vert[i]->row->desc1->atom->acid_start->next->crd,
                    ROTA, trans);
...
double Kabsch::superimpose_calpha(Item* item) {
    ...
    */
    ...
    /*
       nato dodamo unique calpha v X in Y matriko, ce je na primer 2x
       ARG201--ARG345, dodamo samo enkrat calpha,
       ce je ARG201--ARG345 in ARG201--ARG346, potem dodamo dvakrat calpha.

       NOVO APR/17/2010 : stevilka residueja ni vec edinstvena, ker imamo lahko
       vec chain-ov, sedaj razlikujemo residueje po acid_start
    */

    set<pair<Atom*, Atom*> >
        unique_resi;  // residueje razlikujemo po acid_start
    ...
    for (unsigned int i = 0; i < item->vert.size(); i++) {
        ...
        /* upostevamo le en calpha, tudi ce vec deskriptorjev kaze nanj */
        if (unique_resi.find(make_pair(acid_start1, acid_start2)) ==
            unique_resi.end()) {
            unique_resi.insert(make_pair(acid_start1, acid_start2));
            gsl_vector_set(
                vec, 0,
                item->vert[i]->row->desc1->atom->acid_start->next->crd.x);
            gsl_vector_set(
                vec, 1,
                item->vert[i]->row->desc1->atom->acid_start->next->crd.y);
            gsl_vector_set(
                vec, 2,
                item->vert[i]->row->desc1->atom->acid_start->next->crd.z);

            gsl_matrix_set_row(X, st, vec);

            gsl_vector_set(
                vec, 0,
                item->vert[i]->row->desc2->atom->acid_start->next->crd.x);
            gsl_vector_set(
                vec, 1,
                item->vert[i]->row->desc2->atom->acid_start->next->crd.y);
            gsl_vector_set(
                vec, 2,
                item->vert[i]->row->desc2->atom->acid_start->next->crd.z);
    ...
    /* ce prileganje uspe, alignamo calpha atome z izracunano matriko ROTA in
     * trans in izracunamo RMSD med calpha */
...
double Kabsch::superimpose_calpha_single_desc(Item* item) {
    /*
      Funkcija prilega deskriptorje (enega na vsak par prileganih ak, torej ce
      je
...
double Kabsch::superimpose_single_desc(Item* item) {
    /*
      Funkcija prilega deskriptorje (enega na vsak par prileganih ak, torej ce
      je
      vec, npr ARG13-ARG130 deskriptorjev vzame le enega izmed njih - na ta
      nacin
    ...
    */
    ...
    /* ce prileganje uspe, alignamo deskriptorje z izracunano matriko ROTA in
     * trans in izracunamo RMSD med deskriptorji */
    if (kabsch(st, X, Y, ROTA, trans, NULL) == 1) {
        double rmsd = 0.0;
        ...
        for (unsigned int i = 0; i < item->vert.size(); i++) {
            ...
            if (unique_resi.find(make_pair(resi1, resi2)) ==
                ...
                Coor c =
                    rotate_vector(item->vert[i]->row->desc1->crd, ROTA, trans);
...
double Kabsch::superimpose_only_calpha(Item* item) {
    ...
    */
    ...
    /*
       dodamo unique calpha v X in Y matriko, ce je na primer 2x ARG201--ARG345,
       dodamo samo enkrat calpha,
       ce je ARG201--ARG345 in ARG201--ARG346, potem dodamo dvakrat calpha.
    */
    set<pair<int, int> > unique_resi;
    unique_resi.clear();
    ...
    for (unsigned int i = 0; i < item->vert.size(); i++) {
        ...
        if (unique_resi.find(make_pair(resi1, resi2)) == unique_resi.end()) {
            ...
            if (one_letter_code(item->vert[i]->row->desc1->atom->resn) != 'G' &&
                ...
                gsl_vector_set(vec, 0, item->vert[i]
                                           ->row->desc1->atom->acid_start->next
                                           ->next->next->next->crd.x);
                gsl_vector_set(vec, 1, item->vert[i]
                                           ->row->desc1->atom->acid_start->next
                                           ->next->next->next->crd.y);
                gsl_vector_set(vec, 2, item->vert[i]
                                           ->row->desc1->atom->acid_start->next
                                           ->next->next->next->crd.z);

                gsl_matrix_set_row(X, st, vec);

                gsl_vector_set(vec, 0, item->vert[i]
                                           ->row->desc2->atom->acid_start->next
                                           ->next->next->next->crd.x);
                gsl_vector_set(vec, 1, item->vert[i]
                                           ->row->desc2->atom->acid_start->next
                                           ->next->next->next->crd.y);
...
bool Kabsch::compare_matrices(gsl_matrix* U1, gsl_matrix* U2, gsl_vector* t1,
                              gsl_vector* t2) {
    if (fabs(gsl_matrix_get(U1, 0, 0) - gsl_matrix_get(U2, 0, 0)) <
            MATRIX_CUTOFF &&
        fabs(gsl_matrix_get(U1, 0, 1) - gsl_matrix_get(U2, 0, 1)) <
            MATRIX_CUTOFF &&
        fabs(gsl_matrix_get(U1, 0, 2) - gsl_matrix_get(U2, 0, 2)) <
            MATRIX_CUTOFF &&
        fabs(gsl_matrix_get(U1, 1, 0) - gsl_matrix_get(U2, 1, 0)) <
            MATRIX_CUTOFF &&
        fabs(gsl_matrix_get(U1, 1, 1) - gsl_matrix_get(U2, 1, 1)) <
            MATRIX_CUTOFF &&
        fabs(gsl_matrix_get(U1, 1, 2) - gsl_matrix_get(U2, 1, 2)) <
            MATRIX_CUTOFF &&
        fabs(gsl_matrix_get(U1, 2, 0) - gsl_matrix_get(U2, 2, 0)) <
            MATRIX_CUTOFF &&
        fabs(gsl_matrix_get(U1, 2, 1) - gsl_matrix_get(U2, 2, 1)) <
            MATRIX_CUTOFF &&
        fabs(gsl_matrix_get(U1, 2, 2) - gsl_matrix_get(U2, 2, 2)) <
            MATRIX_CUTOFF &&
...
//        gsl_vector_sub(y, y1);
//        gsl_vector_mul(y, y);
//        rez += sqrt(gsl_vector_get(y, 0) + gsl_vector_get(y, 1) +
//        gsl_vector_get(y, 2));
//      }
//      gsl_vector_free(y);
//      gsl_vector_free(y1);
...
Coor Kabsch::rotate_vector(Coor crd, gsl_matrix* ROTA, gsl_vector* trans) {
    /*
      Rotate a vector in crd according to ROTA and trans and return the rotated
      vector.
    */
    Coor c;
    gsl_vector* vec = gsl_vector_alloc(3);
...
Coor Kabsch::rotate_vector_inv(Coor crd, gsl_matrix* ROTA, gsl_vector* trans) {
    /*
      Inversely rotate a vector in crd according to ROTA and trans and return
      the rotated vector.
      The problem is that in mysql are stored rotation and translation of
      PROTEIN1 to PROTEIN2. To rotate
      PROTEIN2 to PROTEIN1, the equation has to be inverted to get vec:
      ROTA*vec+trans = y | * A^(-1)
      A^(-1)=A(transposed) , therefore to get back the initial vector, vec =
      ROTA(transposed) * (y - trans)
    */
    Coor c;
    gsl_vector* vec = gsl_vector_alloc(3);
    gsl_vector* y = gsl_vector_alloc(3);
...
void Kabsch::output(gsl_matrix* ROTA, gsl_vector* trans) {
    ...
    */
    cout << "Vektor = (" << gsl_vector_get(trans, 0) << ","
         << gsl_vector_get(trans, 1) << "," << gsl_vector_get(trans, 2) << ")"
         << endl;
    cout << "Matrika = (" << gsl_matrix_get(ROTA, 0, 0) << ","
         << gsl_matrix_get(ROTA, 0, 1) << "," << gsl_matrix_get(ROTA, 0, 2)
         << ")" << endl;
    cout << "          (" << gsl_matrix_get(ROTA, 1, 0) << ","
         << gsl_matrix_get(ROTA, 1, 1) << "," << gsl_matrix_get(ROTA, 1, 2)
         << ")" << endl;
    cout << "          (" << gsl_matrix_get(ROTA, 2, 0) << ","
         << gsl_matrix_get(ROTA, 2, 1) << "," << gsl_matrix_get(ROTA, 2, 2)
         << ")" << endl;
}
...
// void Kabsch::set_rota_trans(gsl_matrix *ROTA, gsl_vector *trans,
// vector<string> row) {
//  /*
//    Nastavimo rotacijsko matriko in vektor iz char arraya.
//  */
