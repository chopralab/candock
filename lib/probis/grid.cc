...
//~ Grid::Grid(pair<Coor, Coor> pc) : minCrd(pc.first), maxCrd(pc.second), outsideGrid(false) {
//~ 
  //~ dbgmsg("GRID> Allocating memory and determining size of grid" );
  //~ clock_t start = clock();
//~ 
  //~ /* najprej dolocimo stevilo celic, vzamemo nekoliko vecjo kocko okoli proteina zaradi ligandov */
...
Grid::~Grid() {
  ...
  if (outsideGrid) dbgmsg("Warning (GRID) : Some spheres were outside grid (normal with --lig) ! "); 
  ...
  for (x = mx; x <= nx; x++) 
    for (y = my; y <= ny; y++) 
      for (z = mz; z <= nz; z++) {
        ...
        while (tmp != NULL) {
          sphere2 = tmp->item;
          if (dist(sphere->crd, sphere2->crd) < sphere->r + distance + sphere2->r)
            if (sphere2->num != sphere->num) {
              tmp2 = new EElement();
              tmp2->item = sphere2;
              tmp2->next = ((Atom*)sphere)->neighb;
              ((Atom*)sphere)->neighb = tmp2;
//              dbgmsg("sphere->num = "<< sphere->num << " x = " << sphere->crd.x << "sphere2->num = "<< sphere2->num << " x = " << sphere2->crd.x);
            }
          tmp = tmp->next;
        }
      }
  ...
  for (x = mx; x <= nx; x++) 
    for (y = my; y <= ny; y++) 
      for (z = mz; z <= nz; z++) {
        ...
        while (tmp != NULL) {
          sphere2 = tmp->item;
          //          if (((Descriptor*)sphere2)->colors.is_visited(color))
//          dbgmsg("sphere->r = " << sphere->r << " sphere2->r = " << sphere2->r);


          // pri deskriptorjih je sphere->r in sphere2->r je 0
          if (sqrt(dAB = dist_fast(sphere->crd, sphere2->crd)) < sphere->r + distance + sphere2->r)
            if (sphere2->num != sphere->num) {
              tmp2 = new Element();
              tmp2->item = sphere2;
              tmp2->dAB = dAB;
...
void Grid::volume_slice_lig(Sphere *sphere, double max_dist, double distance) {
  /*
    Naredimo neighb listo za Ligand class. Sosedi so le ligandi istega tipa kot centralni ligand.
  */
  int x,y,z;
  int mx,my,mz,nx,ny,nz;
  Sphere *sphere2;
  ...
  for (x = mx; x <= nx; x++) 
    for (y = my; y <= ny; y++) 
      for (z = mz; z <= nz; z++) {
        ...
        while (tmp != NULL) {
          sphere2 = tmp->item;
          if (sqrt(dAB = dist_fast(sphere->crd, sphere2->crd)) < distance)
            // pointerji, ker num pri ligandih ni definiran, preverimo tip liganda, sosedi so istega tipa
            if (sphere2 != sphere && ((Ligand*)sphere2)->type == ((Ligand*)sphere)->type) {
              tmp2 = new Element();
              tmp2->item = sphere2;
              tmp2->dAB = dAB;
        ...
        while (tmp != NULL) {
          ...
          if (dist(probe->crd, atom2->crd) < distance + atom2->r)
            if (!atom2->colors.is_visited(probe->color)) {
              atom2->colors.color[atom2->colors.size++] = probe->color;
            }
          tmp = tmp->next;
        }
      }
