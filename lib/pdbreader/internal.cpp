...
namespace Molib {
	...
	//~ void Internal::build(const MolGraph &graph) {
		//~ for (auto &v1 : graph) {	// find all bonds lengths
			//~ for (auto &v2 : v1) {
				//~ const Atom* a1 = &v1;
				//~ const Atom* a2 = &v2;
				//~ __ic_bond[a1][a2] = a1->crd().distance(a2->crd());
				//~ dbgmsg("bond length between atoms " << a1->atom_number() << " and " << a2->atom_number() << " is " << __ic_bond[a1][a2]);
				//~ for (auto &v3 : v2) {  // find all angles
					//~ if (&v1 != &v3) {
						//~ const Atom* a3 = &v3;
						//~ __ic_angle[a1][a2][a3] = Geom3D::angle(a1->crd(), a2->crd(), a3->crd());
						//~ dbgmsg("angle between atoms " << a1->atom_number() << ", " << a2->atom_number() << " and " << a3->atom_number() << " is " << Geom3D::degrees(__ic_angle[a1][a2][a3]));
						//~ for (auto &v4 : v3) {  // find all torsional angles
							//~ if (&v1 != &v4 && &v2 != &v4) {
								//~ const Atom* a4 = &v4;
								//~ __ic_dihedral[a1][a2][a3][a4] = Geom3D::dihedral(a1->crd(), a2->crd(), a3->crd(), a4->crd());
								//~ dbgmsg("dihedral between atoms " << a1->atom_number() << ", " << a2->atom_number() << ", " << a3->atom_number() << " and " << a4->atom_number() << " is " << Geom3D::degrees(__ic_dihedral[a1][a2][a3][a4]));
							//~ }
						//~ }
					//~ }
				//~ }
