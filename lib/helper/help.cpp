...
std::tuple<double, double, double> gnuplot(const double& x1, const double& x2,
    ...
    for (double a = 1e+5; a < 1e+10; a *= 2) {
        // fit 1/x^12 repulsion term onto datapoints formatted x y
        // (one-per-line)
        const string cmd =
            "gnuplot << EOF 2>&1\n"
            "f(x) = a/x**12 + b\n"
        ...
        while (!feof(pipe)) {
            if (fgets(buffer, 128, pipe) != NULL) output.push_back(buffer);
        }
        pclose(pipe);
...
vector<vector<string>> get_replacement(const vector<string>& initial) {
    ...
    if (initial.size() == 2) {
        ...
        if (gaff_replacement.count(iniclass1) == 0) return result;
        for (auto& repclass1 : gaff_replacement.at(iniclass1)) {
            if (gaff_replacement.count(iniclass2) == 0) continue;
        ...
        for (auto& repclass1 : gaff_replacement.at(iniclass1)) {
            if (gaff_replacement.count(iniclass2) == 0) continue;
            for (auto& repclass2 : gaff_replacement.at(iniclass2)) {
                if (gaff_replacement.count(iniclass3) == 0) continue;
        ...
        for (auto& repclass1 : gaff_replacement.at(iniclass1)) {
            ...
            for (auto& repclass2 : gaff_replacement.at(iniclass2)) {
                if (gaff_replacement.count(iniclass3) == 0) continue;
                for (auto& repclass3 : gaff_replacement.at(iniclass3)) {
                    if (gaff_replacement.count(iniclass4) == 0) continue;
                    for (auto& repclass4 : gaff_replacement.at(iniclass4)) {
                        if (!(repclass1 == iniclass1 &&
                              repclass2 == iniclass2 &&
                              repclass3 == iniclass3 &&
                              repclass4 == iniclass4)) {
                            dbgmsg("replacement dihedral type ["
                                   ...
                                   << repclass3 << " " << repclass4 << "]");
                            result.push_back(
                                {repclass1, repclass2, repclass3, repclass4});
                        }
                    }
                }
            }
...
const map<const string, const map<string, string>> standard_residues{
    {"A", {{"P", "Pac"},  {"OP1", "O3-"}, {"OP2", "O3-"}, {"O5'", "O3"},
           {"C5'", "C3"}, {"C4'", "C3"},  {"O4'", "O3"},  {"C3'", "C3"},
           {"O3'", "O3"}, {"C2'", "C3"},  {"O2'", "O3"},  {"C1'", "C3"},
           {"N1", "Npl"}, {"N3", "Npl"},  {"N6", "Npl"},  {"N7", "Npl"},
           {"N9", "Npl"}, {"C2", "Car"},  {"C4", "Car"},  {"C5", "Car"},
           {"C6", "Car"}, {"C8", "Car"}}},
    {"C", {{"P", "Pac"},  {"OP1", "O3-"}, {"OP2", "O3-"}, {"O5'", "O3"},
           {"C5'", "C3"}, {"C4'", "C3"},  {"O4'", "O3"},  {"C3'", "C3"},
           {"O3'", "O3"}, {"C2'", "C3"},  {"O2'", "O3"},  {"C1'", "C3"},
           {"N1", "Npl"}, {"N3", "Npl"},  {"N4", "Npl"},  {"C2", "Car"},
           {"C4", "Car"}, {"C5", "Car"},  {"C6", "Car"},  {"O2", "O2"}}},
    {"G", {{"P", "Pac"},  {"OP1", "O3-"}, {"OP2", "O3-"}, {"O5'", "O3"},
           {"C5'", "C3"}, {"C4'", "C3"},  {"O4'", "O3"},  {"C3'", "C3"},
           {"O3'", "O3"}, {"C2'", "C3"},  {"O2'", "O3"},  {"C1'", "C3"},
           {"N1", "Npl"}, {"N2", "Npl"},  {"N3", "Npl"},  {"N7", "Npl"},
           {"N9", "Npl"}, {"C2", "Car"},  {"C4", "Car"},  {"C5", "Car"},
           {"C6", "Car"}, {"C8", "Car"},  {"O6", "O2"}}},
    {"U", {{"P", "Pac"},  {"OP1", "O3-"}, {"OP2", "O3-"}, {"O5'", "O3"},
           {"C5'", "C3"}, {"C4'", "C3"},  {"O4'", "O3"},  {"C3'", "C3"},
           {"O3'", "O3"}, {"C2'", "C3"},  {"O2'", "O3"},  {"C1'", "C3"},
           {"N1", "Npl"}, {"N3", "Npl"},  {"C2", "Car"},  {"C4", "Car"},
           {"C5", "Car"}, {"C6", "Car"},  {"O2", "O2"},   {"O4", "O2"}}},
    {"ALA",
     {{"OXT", "O2-"},
      {"CA", "C3"},
      {"CB", "C3"},
...
const map<const string, const map<string, string>> cofactor_residues{
    {"FAD", {{"PA", "Pac"},  {"O1A", "O3-"}, {"O2A", "O3-"}, {"O5B", "O3"},
             {"C5B", "C3"},  {"C4B", "C3"},  {"O4B", "O3"},  {"C3B", "C3"},
             {"O3B", "O3"},  {"C2B", "C3"},  {"O2B", "O3"},  {"C1B", "C3"},
             {"N9A", "Npl"}, {"C8A", "Car"}, {"N7A", "N2"},  {"C5A", "Car"},
             {"C6A", "Car"}, {"N6A", "Npl"}, {"N1A", "N2"},  {"C2A", "Car"},
             {"N3A", "N2"},  {"C4A", "Car"}, {"N1", "N2"},   {"C2", "Car"},
             {"O2", "O2"},   {"N3", "Npl"},  {"C4", "Car"},  {"O4", "O2"},
             {"C4X", "Car"}, {"N5", "N2"},   {"C5X", "Car"}, {"C6", "Car"},
             {"C7", "Car"},  {"C7M", "C3"},  {"C8", "Car"},  {"C8M", "C3"},
             {"C9", "Car"},  {"C9A", "Car"}, {"N10", "Npl"}, {"C10", "Car"},
             {"C1'", "C3"},  {"C2'", "C3"},  {"O2'", "O3"},  {"C3'", "C3"},
             {"O3'", "O3"},  {"C4'", "C3"},  {"O4'", "O3"},  {"C5'", "C3"},
             {"O5'", "O3"},  {"P", "Pac"},   {"O1P", "O3-"}, {"O2P", "O3-"},
             {"O3P", "O3"}}},
    {"NAD", {{"PA", "Pac"},  {"O1A", "O3-"}, {"O2A", "O3-"}, {"O5B", "O3"},
             {"C5B", "C3"},  {"C4B", "C3"},  {"O4B", "O3"},  {"C3B", "C3"},
             {"O3B", "O3"},  {"C2B", "C3"},  {"O2B", "O3"},  {"C1B", "C3"},
             {"N9A", "Npl"}, {"C8A", "Car"}, {"N7A", "N2"},  {"C5A", "Car"},
             {"C6A", "Car"}, {"N6A", "Npl"}, {"N1A", "N2"},  {"C2A", "Car"},
             {"N3A", "N2"},  {"C4A", "Car"}, {"O3", "O3"},   {"PN", "Pac"},
             {"O1N", "O3-"}, {"O2N", "O3-"}, {"O5D", "O3"},  {"C5D", "C3"},
             {"C4D", "C3"},  {"O4D", "O3"},  {"C3D", "C3"},  {"O3D", "O3"},
             {"C2D", "C3"},  {"O2D", "O3"},  {"C1D", "C3"},  {"N1N", "Ng+"},
             {"C2N", "Car"}, {"C3N", "Car"}, {"C7N", "C2"},  {"O7N", "O2"},
             {"N7N", "Npl"}, {"C4N", "Car"}, {"C5N", "Car"}, {"C6N", "Car"}}},
    {"NAP", {{"PA", "Pac"},  {"O1A", "O3-"}, {"O2A", "O3-"}, {"O5B", "O3"},
             {"C5B", "C3"},  {"C4B", "C3"},  {"O4B", "O3"},  {"C3B", "C3"},
             {"O3B", "O3"},  {"C2B", "C3"},  {"O2B", "O3"},  {"P2B", "Pac"},
             {"O1X", "O3-"}, {"O2X", "O3-"}, {"O3X", "O3-"}, {"C1B", "C3"},
             {"N9A", "Npl"}, {"C8A", "Car"}, {"N7A", "N2"},  {"C5A", "Car"},
             {"C6A", "Car"}, {"N6A", "Npl"}, {"N1A", "N2"},  {"C2A", "Car"},
             {"N3A", "N2"},  {"C4A", "Car"}, {"O3", "O3"},   {"PN", "Pac"},
             {"O1N", "O3-"}, {"O2N", "O3-"}, {"O5D", "O3"},  {"C5D", "C3"},
             ...
             {"C2N", "Car"}, {"C3N", "Car"}, {"C7N", "C2"},  {"O7N", "O2"},
             {"N7N", "Npl"}, {"C4N", "Car"}, {"C5N", "Car"}, {"C6N", "Car"}}},
    {"HEM", {{"FE", "Fe"},   {"CHA", "C2"},  {"CHB", "C2"},  {"CHC", "C2"},
             {"CHD", "C2"},  {"NA", "Npl"},  {"C1A", "Car"}, {"C2A", "Car"},
             {"C3A", "Car"}, {"C4A", "Car"}, {"CMA", "C3"},  {"CAA", "C3"},
             {"CBA", "C3"},  {"CGA", "Cac"}, {"O1A", "O2-"}, {"O2A", "O2-"},
             {"NB", "N2"},   {"C1B", "Car"}, {"C2B", "Car"}, {"C3B", "Car"},
             {"C4B", "Car"}, {"CMB", "C3"},  {"CAB", "C2"},  {"CBB", "C2"},
             {"NC", "Npl"},  {"C1C", "Car"}, {"C2C", "Car"}, {"C3C", "Car"},
             {"C4C", "Car"}, {"CMC", "C3"},  {"CAC", "C2"},  {"CBC", "C2"},
             {"ND", "N2"},   {"C1D", "Car"}, {"C2D", "Car"}, {"C3D", "Car"},
             {"C4D", "Car"}, {"CMD", "C3"},  {"CAD", "C3"},  {"CBD", "C3"},
             {"CGD", "Cac"}, {"O1D", "O2-"}, {"O2D", "O2-"}}}};
...
vector<string> ssplit(const string& source, const char* delimiter,
                      bool keepEmpty) {
