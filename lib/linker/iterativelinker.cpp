													 ...
													 const Partial &start_conformation, vector<unique_ptr<State>> &states, int iter)
{
	...
	while (!openset.empty())
	{
		...
		else
		{
			...
			{
				auto ret2 = (adj_is_seed ? State::Vec{adj_is_seed} : __compute_neighbors(curr_state, adj, states));
				for (auto &pneighbor : ret2)
				{
					...
					if (!__clashes_ligand(*pneighbor, curr_conformation, curr_state))
					{ // don't check for clashes with receptor
						...
						for (auto &point : __modeler.get_state(next_conformation.get_ligand_atoms()))
							dbgmsg(point);
#endif
...
Partial::Vec Linker::IterativeLinker::__generate_rigid_conformations(const Seed::Graph &seed_graph)
{
	...
	Partial::Vec possibles_w_energy;
	{
		...
		if (qmaxes.empty())
			throw Error("die : couldn't find any possible conformations for ligand " + __ligand.name());
