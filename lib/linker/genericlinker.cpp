/* This is genericlinker.cpp and is part of CANDOCK
 ...
 * segment atoms
 */
void Linker::GenericLinker::__create_states(const Segment::Graph& segment_graph,
    ...
    for (auto& seed_mols : top_seeds) {
        ...
        for (auto& segment : segment_graph) {
            if (segment.get_seed_id() == seed_id) {
                ...
                // create a graph out of the seed "nm" of "to be" ligand
                // molecule
                molib::Atom::Graph gs =
                    molib::Atom::create_graph(seed_mols.first().get_atoms());
                dbgmsg("gs = " << endl << gs);
                molib::Atom::Graph gd =
                    molib::Atom::create_graph(segment.get_atoms());
                dbgmsg("gd = " << endl << gd);
                ...
                // consider ONLY THE FIRST mapping of seed to seed (the symmetry
                // has
                // been taken care of in the rigid docking itself)...
                if (m.size() < 1)
                    throw Error(
                        "die : could not find mapping of docked seed to "
                ...
                for (auto& seed_molecule : seed_mols) {
                    ...
                    const molib::Atom::Vec& seed_atoms =
                        seed_molecule.get_atoms();
                    for (size_t i = 0; i < vertices2.size(); ++i) {
                        ...
                        dbgmsg("adding matched vertex pair "
                               ...
                               << segment.get_atom(vertices1[i]).atom_number()
                               << " are " << crds[vertices1[i]]);
                    }
                    ...
                    if (energy < __max_allow_energy) {
                        ...
                        segment.add_state(unique_ptr<State>(
                            new State(segment, crds, energy)));
                    }
                }
            }
        }
...
Partial::Vec Linker::GenericLinker::init_conformations() {
    log_step << "Starting connection of seeds for ligand " << __ligand.name()
             << endl;
    ...
    dbgmsg("seed graph for ligand " << __ligand.name() << " = "
                                    << __seed_graph);
 ...
 *
 */
DockedConformation::Vec Linker::GenericLinker::compute_conformations(
    ...
    for (auto& partial : partials) {
        ...
        if (!__has_blacklisted(conformation, __blacklist)) {
            try {
                ...
                dbgmsg("connecting ligand "
                       << __ligand.name() << endl
                       << "possible starting conformation is " << endl
                       << grow_link_energy);
                vector<unique_ptr<State>> states;
                docked_conformations.push_back(__a_star(__segment_graph.size(),
                                                        grow_link_energy,
                                                        states, __link_iter));
 ...
 *
 */
DockedConformation::Vec Linker::GenericLinker::__minimize_final(
    DockedConformation::Vec& docked_conformations) {
    ...
    for (auto& docked : docked_conformations) {
        try {
            ...
            if (__max_iterations_pre > 0) {
                OMMIface::Modeler modeler(__modeler.get_forcefield());

                modeler.add_topology(__ligand.get_atoms());
                modeler.init_openmm(__platform, __precision, __accelerators);
                modeler.add_crds(__ligand.get_atoms(),
                                 docked.get_ligand().get_crds());
                ...
                __modeler.add_crds(__receptor.get_atoms(),
                                   docked.get_receptor().get_crds());
                __modeler.add_crds(__ligand.get_atoms(),
                                   modeler.get_state(__ligand.get_atoms()));
            } else {
                __modeler.add_crds(__receptor.get_atoms(),
                                   docked.get_receptor().get_crds());
                __modeler.add_crds(__ligand.get_atoms(),
            ...
            molib::Molecule minimized_receptor(
                __receptor, __modeler.get_state(__receptor.get_atoms()));
            molib::Molecule minimized_ligand(
                __ligand, __modeler.get_state(__ligand.get_atoms()));
            ...
            minimized_conformations.push_back(DockedConformation(
                minimized_ligand, minimized_receptor,
                __score.non_bonded_energy(gridrec, minimized_ligand),
                potential_energy, ++max_clq_identity));
        } catch (OMMIface::Modeler::MinimizationError& e) {
            log_error << "MinimizationError: skipping minimization of one "
...
bool Linker::GenericLinker::__clashes_receptor(const State& current) const {
    for (size_t i = 0; i < current.get_crds().size(); ++i) {
        ...
        if (__gridrec.clashes(molib::Atom(c, a.idatm_type()), __clash_coeff))
            return true;
    }
    return false;
}
...
bool Linker::GenericLinker::__clashes_ligand(const State& current,
    ...
    for (auto& pstate : conformation.get_states()) {
        dbgmsg("clashes_ligand test for state " << current << " and state "
                                                << *pstate);
        if (current.clashes(*pstate, __clash_coeff)) return true;
    }
    return false;
}
...
geometry::Point::Vec Linker::GenericLinker::__rotate(
    const geometry::Quaternion& q, const geometry::Point& p1,
...
State::Vec Linker::GenericLinker::__compute_neighbors(
    const State& curr_state, Segment& next, vector<unique_ptr<State>>& states) {
    ...
    states.push_back(unique_ptr<State>(new State(
        next,
    ...
    dbgmsg("rotate next segment on vector = "
           << crd2 << " - " << crd3 << " by "
           << geometry::degrees(__spin_degrees) << " degree increments");
    const geometry::Quaternion q(
        geometry::Vector3(crd3 - crd2).norm() * sin(__spin_degrees),
        cos(__spin_degrees));
    for (double angle = __spin_degrees; angle < M_PI; angle += __spin_degrees) {
        State& previous_rotated = *states.back();
        states.push_back(unique_ptr<State>(
            new State(next, __rotate(q, crd2, previous_rotated.get_crds()))));
        dbgmsg("rotated state at angle = " << geometry::degrees(angle) << " is "
                                           << *states.back());
    }
...
bool Linker::GenericLinker::__check_distances_to_seeds(
    ...
    for (auto& adjacent_seed_segment : adjacent.get_adjacent_seed_segments()) {
        ...
        if (it != docked_seeds.end() && it->first != &current) {
            ...
            if (distance > mll + __tol_seed_dist ||
                distance < mll - __tol_seed_dist) {
...
string Linker::GenericLinker::to_pdb(const Partial& conformation) {
    ...
    for (auto& pstate : conformation.get_states()) os << pstate->pdb();
    os << "ENDMDL" << endl;
    return os.str();
}

pair<State*, Segment*> Linker::GenericLinker::__find_good_neighbor(
...
Array2d<bool> Linker::GenericLinker::__find_compatible_state_pairs(
    ...
    for (size_t u = 0; u < seed_graph.size(); ++u) {
        ...
        for (size_t v = u + 1; v < seed_graph.size(); ++v) {
            ...
            const double max_linker_length =
                segment1.get_max_linker_length(segment2);
            ...
            dbgmsg("finding compatible state pairs for segments "
                   ...
                   << " with maximum linker length " << max_linker_length
                   << " and "
...
bool Linker::GenericLinker::__has_blacklisted(
    const State::Vec& conformation, const set<State::ConstPair>& blacklist) {
