/* This is segment.cpp and is part of CANDOCK
...
Segment::Segment(const molib::Atom::Vec atoms, const int& seed_id,
    ...
    : __atoms(atoms),
      __seed_id(seed_id),
      __id(idx),
      __join_atom(atoms.size(), false),
      __common_atom(atoms.size(), false) {
    for (size_t i = 0; i < atoms.size(); ++i) __amap[atoms[i]] = i;
}
...
int Segment::adjacent_in_segment(const molib::Atom& atom,
                                 const molib::Atom& forbidden) const {
    for (auto& adj : atom) {
        if (&adj != &forbidden && has_atom(adj)) return get_idx(adj);
    }
    throw Error("die : couldn't find adjacent in segment");
}
...
Segment::Graph Segment::create_graph(const molib::Molecule& molecule) {
    ...
    int idx = 0;
    for (auto& fragment :
         model.get_rigid()) {  // make vertices (segments) of a graph
        dbgmsg(fragment.get_all());
        auto all = fragment.get_all();
        molib::Atom::Vec fragatoms(all.begin(), all.end());
        //~ vertices.push_back(unique_ptr<Segment>(new Segment(fragatoms,
        //fragment.get_seed_id())));
        vertices.push_back(unique_ptr<Segment>(
    ...
    for (size_t i = 0; i < vertices.size(); ++i) {
        ...
        for (size_t j = i + 1; j < vertices.size(); ++j) {
            ...
            dbgmsg(s1.get_atoms().size() << " " << s2.get_atoms().size() << " "
                                         << inter.size());
            if (inter.size() == 1) {
                auto& atom = **inter.begin();
                s1.set_common_atom(atom);
                s2.set_common_atom(atom);
                dbgmsg("intersection size is one for segments " << s1 << " and "
                                                                << s2);
...
Segment::Paths Segment::__find_paths(
    const vector<unique_ptr<Segment>>& segments) {
    /*
    ...
    for (auto& pseg : segments) {
        auto& seg = *pseg;
        if (seg.is_seed()) seeds.insert(&seg);
        if (seg.is_seed() || seg.is_leaf()) seeds_and_leafs.insert(&seg);
    }
    set<Segment::ConstPair> visited;
    for (auto& pseg1 : seeds) {
        for (auto& pseg2 : seeds_and_leafs) {
            if (pseg1 != pseg2 && !visited.count({pseg1, pseg2})) {
                dbgmsg("finding path between " << *pseg1 << " and " << *pseg2);
                ...
                dbgmsg("path between first segment "
                       << *pseg1 << " and second segment " << *pseg2);
            }
        }
    }
    return paths;
    ...
    for (auto& kv : paths) {
        ...
        for (auto it = path.begin(); it != path.end(); ++it) {
            ...
            if (&current != &goal && goal.is_seed()) {
                ...
                if (is_link_adjacent) current.set_adjacent_seed_segments(goal);
                dbgmsg("current = " << current
                                    << " goal.is_seed() = " << boolalpha
            ...
            if (&current != &start && start.is_seed()) {
                ...
                if (is_link_adjacent) current.set_adjacent_seed_segments(start);
                dbgmsg("current = " << current
                                    << " start.is_seed() = " << boolalpha
