/* This is staticlinker.cpp and is part of CANDOCK
...
//~ #define MOVIE
        ...
        inout::output_file(
            molib::Molecule::print_complex(docked.get_ligand(),
                                           docked.get_receptor(),
                                           docked.get_energy()),
            "movie.pdb",
        ...
        if (curr_conformation.size() == segment_graph_size) {
            ...
            if (adj_is_seed ||
                ...
                for (auto& pneighbor : ret2) {
                    ...
                    dbgmsg("clashes_receptor = "
                           << boolalpha << __clashes_receptor(*pneighbor));
                    ...
                    if (!__clashes_ligand(*pneighbor, curr_conformation,
                        ...
                        // compute energy with original (static) receptor
                        // structure, use expensive non_bonded_energy
                        // calculation only for linkers
                        const double energy =
...
#ifndef NDEBUG
                        // TEST if energy determined in a different way is the
                        // same
                        double ene = 0.0, ene2 = 0.0;
                        for (auto& pstate : next_conformation.get_states()) {
                            if (pstate->get_energy() == 0) {
                                ene += __score.non_bonded_energy(
                                    __gridrec,
                                    pstate->get_segment().get_atoms(),
                                    pstate->get_crds());
                            } else {
    ...
    dbgmsg("ENERGY OF MINIMIZED LIGAND (reconstructed) = "
           << __score.non_bonded_energy(__gridrec, mini.get_ligand()));
    dbgmsg("ENERGY OF MINIMIZED LIGAND (calculated) = "
           << __score.non_bonded_energy(__gridrec,
                                        min_conformation.get_ligand_atoms(),
    ...
    for (auto& pstate : min_conformation.get_states()) {
        dbgmsg("seed " << pstate->get_segment().get_seed_id()
                       << " original E = " << pstate->get_energy()
                       << " calculated E = "
                       << __score.non_bonded_energy(
                              __gridrec, pstate->get_segment().get_atoms(),
                              pstate->get_crds()));
    }

#endif
    log_benchmark << "A-STAR for " << __ligand.name() << " took "
...
Partial::Vec Linker::StaticLinker::__generate_rigid_conformations(
    ...
    log_note << "find_compatible_state_pairs took "
    ...
    Partial::Vec possibles_w_energy;
    {
        ...
        dbgmsg("largest possible clique (seed_graph.size()) = "
               << seed_graph.size());
        dbgmsg("currently searched for clique (__max_clique_size) = "
               << __max_clique_size);
        ...
        for (auto& qmax : qmaxes) {
            ...
            for (auto& i : qmax) {
                ss << "seed_" << states[i]->get_segment().get_seed_id()
                   << " state_" << states[i]->get_no() << " ene_"
                   << states[i]->get_energy() << " ";
                compo[states[i]->get_segment().get_seed_id()] =
                    states[i]->get_no();
            }
    ...
    if (__docked_clus_rad <= 0.0) {
        log_benchmark << "Generated " << possibles_w_energy.size()
                      << " possible conformations for ligand without clustering"
...
DockedConformation Linker::StaticLinker::__reconstruct(
    ...
    if (__max_iterations_final == -1)
        return DockedConformation(ligand, __receptor.name(),
                                  conformation.get_energy(), 0, 0);

    return DockedConformation(ligand, __receptor, conformation.get_energy(), 0,
                              0);
}
};
}
