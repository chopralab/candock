/* This is poses.cpp and is part of CANDOCK
        ...
        for (auto& seed : seed_graph) {
            ...
            for (auto& pstate : segment.get_states()) {
                ...
                for (size_t i = 0; i < state.get_crds().size(); ++i) {
                    if (segment.is_join_atom(i))  // FIXME What is meant here?
                                                  // Technically only the debug
                                                  // statement is run, but is
                                                  // this check supposed to run
                                                  // __atompoints too?
                        dbgmsg("pushing to atompoints crd = "
                               << state.get_crd(i)
                               << " atom = " << segment.get_atom(i)
                               << " for STATE = " << state);
                    __atompoints[segment.get_id()].push_back(
                        unique_ptr<AtomPoint>(new AtomPoint(
                            state.get_crd(i), segment.get_atom(i), state)));
                }
            }
            __grid[segment.get_id()] =
                AtomPoint::Grid(__atompoints[segment.get_id()]);
        }
    } catch (...) {
...
State::Set Poses::get_join_states(const State& state, Segment& segment2,
    ...
    for (size_t i = 0; i < state.get_segment().get_atoms().size(); ++i) {
        dbgmsg(
            "atom1 = " << state.get_segment().get_atom(i).atom_number()
                       << " atom2 = " << jatoms.first->atom_number()
                       << " equal = " << boolalpha
                       << (&state.get_segment().get_atom(i) == jatoms.first));
        if (&state.get_segment().get_atom(i) == jatoms.first) {
            dbgmsg("crd = "
                   << state.get_crd(i) << " mll = " << max_linker_length
                   << " lower_tol_seed_dist = " << lower_tol_seed_dist
                   << " upper_tol_seed_dist = " << upper_tol_seed_dist);
            AtomPoint::PVec neighbors =
                g.get_neighbors_within_tolerance_asymmetric(
                    state.get_crd(i), max_linker_length, lower_tol_seed_dist,
                    upper_tol_seed_dist);
