/* This is conformations.cpp and is part of CANDOCK
...
Conformations::Conformations(const molib::Molecule& seed,
                             const Gpoints& gpoints, const double& conf_spin,
                             const int num_univec)
    : __seed(seed) {
    try {
        ...
        geometry::Point::Vec unit_vectors =
            geometry::uniform_sphere(num_univec);
        ...
        for (auto& crd : seed_crds) crd = crd - center.crd();
        ...
        for (auto& unitvec : unit_vectors) {
            geometry::Vector3 ortho =
                geometry::Coordinate::cross(bondvec, unitvec);
            const double rotangl = geometry::angle(unitvec, bondvec) / 2;
            dbgmsg("rotangl = " << geometry::degrees(rotangl)
                                << " seed = " << seed.name());
            const geometry::Quaternion q0(ortho.norm() * sin(rotangl),
                                          cos(rotangl));
            ...
            const geometry::Quaternion q(
                geometry::Vector3(unitvec) * sin(conf_spin_in_radians),
        ...
        for (size_t i = 0; i < confs.size(); ++i) {
            for (size_t j = i + 1; j < confs.size(); ++j) {
                __rmsd_sq.data[i][j] = __rmsd_sq.data[j][i] =
                    geometry::compute_rmsd_sq(confs[i], confs[j]);
            }
        }
        ...
        log_benchmark << "time to find " << __conf_vec.size()
                      << " conformations of seed " << seed.name() << " took "
                      << bench.seconds_from_start() << " wallclock seconds"
                      << "\n";

    } catch (...) {
        dbgmsg(
            "FAILURE: constructor of Conformations failed ... cleaning "
            "resources...");
        throw;
    }
}
