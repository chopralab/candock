/* This is dock.cpp and is part of CANDOCK
...
Dock::DockedConf::Vec Dock::__dock() {
    ...
    for (auto& kv : __gpoints.get_gridpoints()) {
        ...
        for (auto& cavpoint : kv.second) {
            ...
            for (size_t c = 0; c < conformations.size(); ++c) {
                ...
                // test if c-th conformation clashes with receptor: if yes,
                // reject it
                if (!rejected.data[c]) {
                    double energy_sum(0);
                    bool reje = false;
                    ...
                    for (size_t i = 0; i < conf.size(); ++i) {
                        ...
                        docker::Gpoints::IJK confijk =
                            cavpoint.ijk() + gpoint0.ijk();

                        dbgmsg("cavpoint.ijk() = " << cavpoint.ijk());
                        dbgmsg("gpoint.ijk() = " << gpoint0.ijk());
                        dbgmsg("confijk = " << confijk);
                        dbgmsg("gmap.szi = " << gmap.szi
                                             << " gmap.szj = " << gmap.szj
                                             << " gmap.szk = " << gmap.szk);

                        if (confijk.i < 0 || confijk.j < 0 || confijk.k < 0 ||
                            (size_t)confijk.i >= gmap.szi ||
                            (size_t)confijk.j >= gmap.szj ||
                            (size_t)confijk.k >= gmap.szk ||
                            gmap.data[confijk.i][confijk.j][confijk.k] ==
                                nullptr) {
                            // mark as rejected all conformations that have this
                            // point
                            for (auto& r :
                                 __conformations.get_confs_at(gpoint0.ijk())) {
                        ...
                        dbgmsg("energy calculated at crd = "
                               << pgpoint->crd() << " is "
                               << __score.non_bonded_energy(
                                      __gridrec, molib::Atom::Vec{&atom},
                                      geometry::Point::Vec{pgpoint->crd()}));
                    }
                    // if no clashes were found ...
                    if (!reje) {
                        accepted_tmp.push_back(Dock::DockedConf(
                            cavpoint, conf, energy_sum, c, bsite_id));
#ifndef NDEBUG
                        molib::Atom::Vec seed_atoms = __seed.get_atoms();
                        const Gpoints::PGpointVec& points =
                            accepted_tmp.back().get_conf0();
                        ...
                        dbgmsg("ACCEPTED conformation energy = "
                               << accepted_tmp.back().get_energy()
                               << " calculated energy = "
                               << __score.non_bonded_energy(__gridrec, __seed));
#endif
                    }
                }
            }
    ...
    log_benchmark << "Fragment docking for seed " << __seed.name() << " took "
...
void Dock::__cluster_fast(const DockedConf::Vec& conformations,
    ...
    while (!confs.empty()) {
        ...
        for (auto it = confs.begin(); it != confs.end(); ++it) {
            if (__conformations.get_rmsd_sq(lowest_point.get_i(),
                                            (*it)->get_i()) < __rmsd_tol_sq) {
                confs.erase(it);
            }
        }
    }
...
Dock::DockedConf::Vec Dock::__cluster(const DockedConf::Vec& conformations) {
    ...
    molib::Grid<const Dock::DockedConf> cgrid(
    ...
    while (!confs.empty()) {
        ...
        // yconformation
        for (auto& pconf :
             cgrid.get_neighbors(lowest_point.crd(), __rmsd_tol)) {
            if (pconf->compute_rmsd_sq(lowest_point) < __rmsd_tol_sq) {
                confs.erase(pconf);
            }
        }
    }
    log_benchmark << "Clustering accepted conformations for seed "
                  << __seed.name() << " took " << bench.seconds_from_start()
                  << " seconds"
                  << "\n";
    return reps;
}
...
void Dock::__set_docked(const DockedConf::Vec& confs) {
    ...
    for (auto& conf : confs) {
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            ...
            docker::Gpoints::IJK confijk =
                conf.get_cavpoint().ijk() + gpoint0.ijk();
            docker::Gpoints::Gpoint* pgpoint =
                gmap.data[confijk.i][confijk.j][confijk.k];
        ...
        __docked.add(new molib::Molecule(__seed))
            .set_name(
                std::to_string(conf.get_energy()));  // molecule name is energy
        dbgmsg("conformation energy = "
               << conf.get_energy() << " calculated energy = "
               << __score.non_bonded_energy(__gridrec, __seed));
    }
