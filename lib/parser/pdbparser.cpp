...
void FileParser::PdbParser::parse_molecule(Molecules& mols) {
    ...
    vector<string> pdb_raw;
    {
        ...
        Inout::read_stream(__stream, pdb_raw, __num_occur,
                           "REMARK   5 MOLECULE");
    }
    std::smatch m;
    set<char> bio_chain;
    ...
    for (string& line : pdb_raw) {
        ...
        if (line.compare(0, 4, "ATOM") == 0 ||
            ...
            string element =
                line.size() > 77
                    ? boost::algorithm::trim_copy(line.substr(76, 2))
                    : "";
            ...
            string idatm_type =
                line.size() > 80
                    ? boost::algorithm::trim_copy(line.substr(80, 5))
                    : "???";
            idatm_type =
                help::idatm_mask.count(idatm_type) ? idatm_type : "???";
            string gaff_type =
                line.size() > 85
                    ? boost::algorithm::trim_copy(line.substr(85, 5))
                    : "???";
            string rest_of_line =
                line.size() > 90 ? boost::algorithm::trim_copy(line.substr(90))
                                 : "";
            const bool hydrogen =
                (element == "H" ||
                 ((element.empty() || element.at(0) == ' ') &&
                  (atom_name.size() == 1 && atom_name.at(0) == 'H')));
            ...
            if ((__hm & hydrogens) || !hydrogen) {
                if (alt_loc == ' ' || alt_loc == 'A') {
                    ...
                    if (help::amino_acids.find(resn) !=
                        help::amino_acids.end()) {
                        rest = Residue::protein;
                    } else if (help::nucleic_acids.find(resn) !=
                               help::nucleic_acids.end()) {
                        rest = Residue::nucleic;
                    } else if (mols.last().is_modified(Residue::res_tuple2(
                                   chain_id, resn, resi, ins_code))) {
                        rest = Residue::modres;
                    ...
                    if (!__giant_molecule || rest != Residue::protein ||
                        atom_name == "CA") {
                        ...
                        if (!chain->has_residue(
                                Residue::res_pair(resi, ins_code))) {
                            residue = &chain->add(
                                new Residue(resn, resi, ins_code, rest));
                        }

                        Atom& a = residue->add(
                            new Atom(atom_number, atom_name, crd,
                                     help::idatm_mask.at(idatm_type), element));
        ...
        } else if (line.compare(0, 14, "REMARK   6 BIO") == 0 ||
                   line.compare(0, 14, "REMARK   6 ASY") == 0) {
            int number = 0;
            string name = "";

            if (std::regex_search(
                    line, m, std::regex("REMARK   6 (\\S+ \\S+)\\s+(\\d+)"))) {
            ...
            if (std::regex_search(line, m, std::regex("MODEL\\s+(\\d+)"))) {
                if (m[1].matched) {
                    ...
                    __generate_molecule(mols, found_molecule, "");
                    __generate_assembly(mols, found_assembly, 0,
                                        "ASYMMETRIC UNIT");
            ...
            if (std::regex_search(
                ...
                bio_chain.clear();
                           ...
                           line, m,
                           std::regex("REMARK 350 APPLY THE FOLLOWING TO "
                                      "CHAINS:\\s?(\\S{1})?,?\\s?(\\S{1})?,?"
                                      "\\s?(\\S{1})?,?\\s?(\\S{1})?,?\\s?(\\S{"
                                      "1})?,?\\s?(\\S{1})?,?\\s?(\\S{1})?,?\\s?"
                                      "(\\S{1})?,?\\s?(\\S{1})?,?")) ||
                ...
                if (m[1].matched && m[2].matched && m[3].matched &&
                    m[4].matched && m[5].matched && m[6].matched) {
                    if (biomolecule_number == -1) {
                    ...
                    if (rn < 15) {
                        if (row == 1) {
                            mols.last().add_bio_chain(biomolecule_number,
                                                      bio_chain);
                        }

                        mols.last().add_bio_rota(biomolecule_number, rn,
                                                 row - 1,
                                                 make_tuple(a, b, c, w));
                    } else {
            ...
            if (help::amino_acids.find(resn) != help::amino_acids.end()) {
                ...
                // modified 'standard' residue gets saved,e.g., MET --> MSE,
                // glycosylated...
                mols.last().add_modified(
                    Residue::res_tuple2(chain_id, resn_mod, resi, ins_code));
            }
        } else if (line.compare(0, 4, "SITE") == 0 &&
                   std::regex_search(
                       line, m,
                       std::regex("SITE\\s+\\d+\\s+(\\S+)\\s+\\S+(\\s+\\S+\\s?"
                                  "\\S{1}\\s{0,3}\\d{1,4}\\S?)?(\\s+\\S+\\s?"
                                  "\\S{1}\\s{0,3}\\d{1,4}\\S?)?(\\s+\\S+\\s?"
                                  "\\S{1}\\s{0,3}\\d{1,4}\\S?)?(\\s+\\S+\\s?"
                                  "\\S{1}\\s{0,3}\\d{1,4}\\S?)?"))) {
            if (m.empty()) {
                ...
                if (el != *m.begin() && el.matched) {
                    ...
                    if (s.size() > 8) {
                        ...
                        if (help::amino_acids.find(resn) !=
                            ...
                            __generate_molecule(mols, found_molecule, "");
                            mols.last().add_site(
                                site_name, Residue::res_tuple2(chain_id, resn,
                                                               resi, ins_code));
                        }
                    }
                }
            ...
            for (; it != vs.end(); it++) {
                ...
                dbgmsg("ADDING TO REMARK "
                       << "[" << lchain_id << ":" << lresn << ":" << lresi
                       << ":" << lins_code << "] => [" << tok1[0].at(0) << ":"
                       << tok1[1] << ":" << stoi(tok1[2]) << ":"
                       << tok1[3].at(0) << "=" << tok2[0].at(0) << ":"
                       << tok2[1] << ":" << stoi(tok2[2]) << ":"
                       << tok2[3].at(0) << "]");
                model.set_remark(
                    7, Residue::res_tuple2(lchain_id, lresn, lresi, lins_code),
                    make_pair(
                        Residue::res_tuple2(tok1[0].at(0), tok1[1],
                                            stoi(tok1[2]), tok1[3].at(0)),
                        Residue::res_tuple2(tok2[0].at(0), tok2[1],
                                            stoi(tok2[2]), tok2[3].at(0))));
            }
            ...
            model.get_rigid().push_back(
                Fragmenter::Fragment(core, join, seed_id));
#ifndef NDEBUG
                ...
                for (auto& model : assembly) {
                    ...
                    if (atom_iterator1 != atom_number_to_atom[&model].end()) {
                        dbgmsg("found atom = "
                               << boolalpha
                               << (atom_iterator1 !=
                                   atom_number_to_atom[&model].end()));
                        Atom& a1 = *atom_iterator1->second;
                        it++;

                        while (it != anum.end()) {
                            auto atom_iterator2 =
                                atom_number_to_atom[&model].find(*it);
                            dbgmsg((atom_iterator2 ==
                                            atom_number_to_atom[&model].end()
                                        ? "warning: atom2 not found (check "
                                          "CONECT words)"
                                        : ""));
                            ...
                            if (atom_iterator2 !=
                                ...
                                // don't allow inter-residue bonds e.g., between
                                // MG and ATP, or between FAD and CYS (issue
                                // #114)
                                if (help::ions.count(resn1) == 0 &&
                                    ...
                                    dbgmsg("Ignoring interresidue bond between "
                                           << resn1 << " and " << resn2
                                           << ". Check if that is what you "
                                              "intended!");
                                }
        ...
        } else if (line.compare(0, 21, "REMARK  20 non-binder") == 0 &&
                   (__hm & docked_poses_only)) {
            return;
        }
    }
}
