...
void FileParser::Mol2Parser::parse_molecule(Molecules& mols) {
    vector<string> mol2_raw;
    {
        ...
        Inout::read_stream(__stream, mol2_raw, __num_occur,
                           "@<TRIPOS>MOLECULE");
    }
    ...
    for (size_t i = 0; i < mol2_raw.size(); ++i) {
        ...
        if (line.find("@<TRIPOS>ATOM") != string::npos) {
            ...
            for (i = i + 1; i < mol2_raw.size(); ++i) {
                ...
                subst_id = 1;        // fix value to prevent multiresidue small
                                     // molecules (issue #110)
                subst_name = "LIG";  // prevent idatm typing for standard
                                     // residues (issue #111)

                const string element = help::sybyl.count(sybyl_type)
                                           ? help::sybyl.at(sybyl_type)
                                           : "";

                subst_name = subst_name.substr(
                    0, 3);  // truncate longer than 3-lett (pde5a bug)
                ...
                dbgmsg("atom_id = "
                       << atom_id << " atom_name = " << atom_name
                       << " x = " << x << " y = " << y << " z = " << z
                       << " sybyl_type = " << sybyl_type << " subst_id = "
                       << subst_id << " subst_name = " << subst_name
                       << " charge = " << charge << " element = " << element
                       << " hydrogen = " << hydrogen);
                ...
                if ((__hm & hydrogens) || !hydrogen) {
                    ...
                    if (!__giant_molecule || rest != Residue::protein ||
                        ...
                        if (!chain->has_residue(
                                Residue::res_pair(subst_id, ' '))) {
                            residue = &chain->add(
                                new Residue(subst_name, subst_id, ' ', rest));
                        }
                        ...
                        Atom& a = residue->add(new Atom(
                            atom_id, atom_name, crd, help::idatm_mask.at("???"),
                            element, sybyl_type));
            ...
            for (i = i + 1; i < mol2_raw.size(); ++i) {
                ...
                for (auto& assembly : molecule) {
                    for (auto& model : assembly) {
                        auto it1 =
                            atom_number_to_atom[&model].find(origin_atom_id);
                        auto it2 =
                            atom_number_to_atom[&model].find(target_atom_id);
