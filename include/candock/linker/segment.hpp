...
namespace linker {
	...
	private:
		...
		static Paths __find_paths(const std::vector<std::unique_ptr<Segment>> &segments);
	...
	public:
		...
		std::string get_label() const { std::stringstream ss; ss << *this; return ss.str(); } // graph ostream operator
		int weight() const { return 0; } // dummy for graph ostream operator
		void add_state(std::unique_ptr<State> s) { __state.push_back(std::move(s)); }
		bool is_seed() const { return __seed_id != -1; }
		bool is_leaf() const { return size() == 1; }
		bool is_branch() const { return size() > 2; }
		const std::vector<std::unique_ptr<State>>& get_states() const { return __state; }
