...
class CANDOCK_EXPORT Logger {
   ...
   public:
    ...
    static void set_all_stderr(bool mode) { __all_to_stderr = mode; }

    static bool all_stderr() { return __all_to_stderr; }

    template <Severity log_type>
    static LoggerImpl<log_type> log() {
        return log_type >= CD_WARNING || all_stderr()
                   ? LoggerImpl<log_type>(std::cerr)
                   : LoggerImpl<log_type>(std::cout);
    }
};
}
}
