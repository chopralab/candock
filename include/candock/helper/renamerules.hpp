...
const rename_rules special{
    ...
    {{{"Npl#1#3,1H#1ag,ag5", "^S|^Oar#2#2#1ag,ag5", ""}},
     {{"1:idatm=N2"}}},  // change 3-substituted Npl bound to S or O in 5-ring
                         // to N2
    {{{"Npl#1#2#1ag,ag5", "^S|^Oar#2#2#1ag,ag5", ""}},
     {{"1:idatm=N2"}}},  // change 2-substituted Npl bound to S or O in 5-ring
                         // to N2
...
const rename_rules refine{
    ...
    {{{"Npl#1#3#sb,db,ag6", ".*#2", ""}},
     {{"1:idatm=N2+"}}},  // change 3-substituted Npl in 6-membered aromatic
                          // ring to N2+
    {{{"Npl#1#3,1H#sb,db", ".*#2", ""}},
...
const rename_rules rotatable{
    ...
    {{{"3$#1", "N2\\+#2", ""}, {"#1", "^[^H]#3", ""}}, {{"1,2:rota=sp3-n2plus,"
                                                         "angles={-150,-90,-30,"
                                                         "30,90,150},drive_id="
                                                         "6"}}},  // added
                                                                  // (issue
    ...
    {{{"2$#1", "2$#2", ""}, {"#1", "^[^H]#3", ""}, {"#2", "^[^H]#4", ""}},
     {{"1,2:rota=sp2-sp2,angles={0,180},drive_id=2"}}},
    {{{"2$#1", "ar$#2", ""}, {"#1", "^[^H]#3", ""}},
     {{"1,2:rota=sp2-aromatic,angles={0,180},drive_id=2"}}},
...
const rename_rules atomic_penalty_scores{
    ...
    {{{"^N#1#3", "^O|^S#2#1", ""}, {"#1", "^O|^S#3#1", ""}},
     {{"1:aps={{3,64},{4,32},{5,0},{6,32}}"}}},  // APSNO2 : for NO2-, NS2-,
                                                 // NOS- etc
    {{{"^N#1#3", "^O|^S#2#1", ""},
      {"#1", "^[^OS]#3", ""},
      {"#1", "^[^OS]#4", ""}},
...
const rename_rules gaff{
    ...
    {{{"^O#1#1", ".*#2", ""}}, {{"1:gaff=o"}}},   // Oxygen with one connected
                                                  // atom (Sp2 oxygen in C=O,
                                                  // COO-)
    ...
    {{{"^S#1#2", ".*#2", ""}}, {{"1:gaff=ss"}}},     // Sp3 S in thio-ester and
                                                  // thio-ether (sp3 sulfur in
                                                  // —SR and S—S)
    ...
    {{{"^S#1#3#db", XD + "#2#4#db", "bond_gaff_type=sb"}},
     {{"1:gaff=sx"}}},  // Special s4 in conjugated systems
    {{{"^S#1#3", ".*#2", ""}},
     {{"1:gaff=s4"}}},  // S with three connected atoms
