/* This is optics.hpp and is part of CANDOCK
...
template <typename T>
using PairwiseDistances = std::map<T*, std::map<T*, double>>;
template <typename T>
using Neighbors = std::map<T*, std::vector<T*>>;
template <typename T>
using MapD = std::map<T*, double>;
template <typename T>
using MapB = std::map<T*, bool>;
template <typename T>
using MapI = std::map<T*, int>;
template <typename T>
using Clusters = std::multimap<int, T*>;
template <typename T>
using Vector = std::vector<T*>;

template <typename T, typename CompareDistanceLess = std::less<double>,
          typename CompareScoreLess = std::less<double>>
class Optics {
    ...
    void __expand_cluster_order(T& p) {
...
#endif
        __processed[&p] = true;
        __reachability_distance[&p] =
            __JANEZ_HV;  // reachability distance is already __JANEZ_HV
        __set_core_distance(p, __min_pts);
        __ordered_file.push_back(&p);
        Vector<T> order_seeds;  // priority_queue sorted by
                                // reachability_distance from the core object
                                // (last element has the smallest
        ...
        if (__core_distance[&p] != __JANEZ_HV) {  // is p a core object?
            ...
            while (!order_seeds.empty()) {
                //~ pop_heap(order_seeds.begin(), order_seeds.end(), by_r_dist);
                //// new
                pop_heap(order_seeds.begin(), order_seeds.end(), [this](T* i,
                                                                        T* j) {
                    //~ return __reachability_distance[i] >
                    //__reachability_distance[j];
                    //~ dbgmsg("i = " << boolalpha << (i == nullptr) << " " <<
                    //*i);
                    //~ dbgmsg("j = " << boolalpha << (j == nullptr) << " " <<
                    //*j);

                    //~ dbgmsg("reachability distance = " <<
                    //__reachability_distance.at(i)
                    //~ << " " << __reachability_distance.at(j));

                    return !CompareDistanceLess()(__reachability_distance[i],
                                                  __reachability_distance[j]);
                });
                ...
                if (__core_distance[&q] != __JANEZ_HV) {  // is q a core object?
                    __update(order_seeds, q);  // if yes, insert further
                                               // candidates into the
                                               // order_seeds
                }
            }
    ...
    void __update(Vector<T>& order_seeds, T& p) {  // p is center object
        ...
        for (auto& po : __neighbors[&p]) {
            ...
            if (!__processed[&o]) {
                ...
                //o.distance() is the distance between o and p
                double new_r_dist =
                    max(c_dist, __distance[&p][&o],
                        CompareDistanceLess());  // o.distance() is the distance
                                                 // between o and p
                                                 //~ #ifndef NDEBUG
                //~ dbgmsg("c_dist =" << c_dist << "\to.distance() =" <<
                //__distance[&p][&o]);
                //~ #endif
                if (__reachability_distance[&o] == __JANEZ_HV) {
                    ...
                    push_heap(order_seeds.begin(), order_seeds.end(),
                              [this](T* i, T* j) {
                                  //~ return __reachability_distance[i] >
                                  //__reachability_distance[j];
                                  //~ dbgmsg("i = " << boolalpha << (i ==
                                  //nullptr) << " " << *i);
                                  //~ dbgmsg("j = " << boolalpha << (j ==
                                  //nullptr) << " " << *j);
                                  ...
                                  return !CompareDistanceLess()(
                                      __reachability_distance[i],
                                      __reachability_distance[j]);
                              });
                    ...
                    if (CompareDistanceLess()(new_r_dist,
                                              __reachability_distance[&o])) {
                        //~ #ifndef NDEBUG
                        ...
                        //~ #endif
                        __reachability_distance[&o] =
                            new_r_dist;  // o is pointed to from the order_seeds
                                         //~ #ifndef NDEBUG
                        ...
                        //~ sort(order_seeds.begin(), order_seeds.end(),
                        //by_r_dist);  // old
                        //~ #ifndef NDEBUG
    ...
    Optics(PairwiseDistances<T>& pairwise_distances, const MapD<T>& scores,
        ...
        for (auto& kv1 : __distance) {
            ...
            sort(__neighbors[p1].begin(), __neighbors[p1].end(),
                 ...
                 {
                     return CompareDistanceLess()(__distance[p1][i],
                                                  __distance[p1][j]);
                 });
            //~ #ifndef NDEBUG
        ...
        for (auto& kv : __distance) {
            auto& p = kv.first;
            if (!__processed[p]) __expand_cluster_order(*p);
        }
        log_benchmark << "total time required for clustering was "
                      << bench.seconds_from_start() << " wallclock seconds\n";
    }

    std::pair<Clusters<T>, Clusters<T>> extract_dbscan(
        const double eps_cur, const size_t max_num_clus = 9999999,
        const bool do_sort = false) {  // returns the original objects together
                                       // with their cluster_ids
        ...
        for (auto& pp : __ordered_file) {
            if (__cluster_id[pp] == -1) {
                ...
                if (!clustered_neighbors.empty()) {
                    T* nearest = *min_element(
                        clustered_neighbors.begin(), clustered_neighbors.end(),
                        [&pp, this](T* i, T* j) {
                            return CompareDistanceLess()(__distance[pp][i],
                                                         __distance[pp][j]);
                        });
                    if (CompareDistanceLess()(__distance[pp][nearest],
                                              eps_cur)) {
                        __cluster_id[pp] = __cluster_id[nearest];
                        dbgmsg("pass 1 : assigned point "
                               << *pp << " to cluster " << __cluster_id[pp]
                               << " because it was close (distance="
                               << __distance[pp][nearest] << ") to point "
                               << *nearest);
                        continue;
                    }
                }
                // pass 2 : find points that are close and assign them to a new
                // cluster
                ...
                for (auto& n : close_neighbors) {
                    ...
                    dbgmsg("pass 2 : assigned point "
                           << *n << " to a new cluster " << __cluster_id[n]);
                }
            }
        }
        if (do_sort) {
#ifndef NDEBUG
            for (auto& pp : __ordered_file) {
                dbgmsg("before sorting cluster_id = "
                       << __cluster_id[pp] << " score = " << __scores.at(pp));
                dbgmsg("pp = " << *pp);
            }
#endif
            ...
            for (size_t i = 0; i < __ordered_file.size(); ++i) {
                for (size_t j = i + 1; j < __ordered_file.size(); ++j) {
                    //~ dbgmsg("before2 sorting : " << *__ordered_file[i] << " "
                    //<< *__ordered_file[j]);
                    if (CompareScoreLess()(__scores.at(__ordered_file[i]),
                                           __scores.at(__ordered_file[j]))) {
...
#ifndef NDEBUG
            for (auto& pp : __ordered_file) {
                dbgmsg("after sorting cluster_id = "
                       << __cluster_id[pp] << " score = " << __scores.at(pp));
            }
#endif
        }
        ...
        for (auto& pp : __ordered_file) {
            ...
            if (added_clus.size() < max_num_clus ||
                added_clus.count(cluster_id)) {
        ...
        for (typename Clusters<T>::iterator it = clustered_points.begin(),
                                            end = clustered_points.end();
             it != end; it = clustered_points.upper_bound(it->first)) {
            auto range = clustered_points.equal_range(it->first);
            T* rep = max_element(range.first, range.second,
                                 [this](const std::pair<const int, T*>& i,
                                        const std::pair<const int, T*>& j) {
                                     return CompareScoreLess()(
                                         __scores.at(i.second),
                                         __scores.at(j.second));
                                 })
                         ->second;
            representative_points.insert({it->first, rep});
            dbgmsg("\t[" << it->first << "]\t" << *rep
                         << "\tscore=" << __scores.at(rep));
        }
#ifndef NDEBUG
        dbgmsg("clusters : ");
        for (auto pp : __ordered_file) {
            dbgmsg("\t[" << __cluster_id[pp] << "]\t" << *pp
                         << "\tcore_distance=" << __core_distance[pp]
                         << "\treachability_distance="
                         << __reachability_distance[pp]);
        }
#endif
