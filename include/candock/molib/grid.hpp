...
class Grid {
   ...
   public:
    ...
    Grid(const P& points) : storage(nullptr), szi(0), szj(0), szk(0) {
        ...
        if (!points.empty()) {
            ...
            for (auto& point : points) {
                if (point->crd().x() < min_crd.x())
                    min_crd.set_x(point->crd().x());

                if (point->crd().y() < min_crd.y())
                    min_crd.set_y(point->crd().y());

                if (point->crd().z() < min_crd.z())
                    min_crd.set_z(point->crd().z());

                if (point->crd().x() > max_crd.x())
                    max_crd.set_x(point->crd().x());

                if (point->crd().y() > max_crd.y())
                    max_crd.set_y(point->crd().y());

                if (point->crd().z() > max_crd.z())
                    max_crd.set_z(point->crd().z());
            }

            __min_crd = min_crd;
            geometry::Coordinate corr =
                max_crd - min_crd;  // calculate size of grid
            szi = corr.i() + 1;
            szj = corr.j() + 1;
            szk = corr.k() + 1;
    ...
    template <typename U>
    Points get_neighbors_within_tolerance_asymmetric(const U& point,
                                                     const double& dist,
                                                     const double& lower_tol,
                                                     const double& upper_tol) {
        ...
        for (int i = cmin.i(); i <= cmax.i(); ++i)
            for (int j = cmin.j(); j <= cmax.j(); ++j)
                for (int k = cmin.k(); k <= cmax.k(); ++k) {
                    for (auto neighbor : storage[i][j][k]) {
                        const double d_sq =
                            atom.crd().distance_sq(neighbor->crd());
                        const double vdw2 = neighbor->radius();

                        if (d_sq < pow(clash_coeff * (vdw1 + vdw2), 2) &&
                            d_sq > 0) {
                            points.push_back(neighbor);
                        }
                    }
