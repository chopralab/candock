   ...
   public:
    Bond()
        : __atom1(nullptr),
          ...
          __ring(false),
          __owns_atoms(false),
          __angles(std::set<int>()),
          __drive_id(0) {}
    Bond(Atom* atom1, Atom* atom2, bool owns_atoms = false)
        : __atom1(atom1),
          __atom2(atom2),
          __idx1(0),
          __idx2(0),
          ...
          __bo(0),
          __ring(false),
          __owns_atoms(owns_atoms),
          __angles(std::set<int>()),
          __drive_id(0) {}
    Bond(Atom* atom1, Atom* atom2, int idx1, int idx2, bool owns_atoms = false)
        : __atom1(atom1),
          __atom2(atom2),
          __idx1(idx1),
          __idx2(idx2),
    ...
    void set_rotatable(const std::string& rotatable) {
        __rotatable = rotatable;
    }
    ...
    void set_bond_gaff_type(const std::string& bond_gaff_type) {
...
std::map<std::string, int> decode_smiles_prop(
