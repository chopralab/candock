...
class Molecule : public template_map_container<Assembly, Molecule, Molecules> {
   ...
   public:
    ...
    bool is_modified(Residue::res_tuple2 t2) const {
        return (__modified.find(t2) != __modified.end());
    }
    void add_site(const std::string& name, Residue::res_tuple2 t) {
        __site.insert(make_pair(name, t));
    }
    void set_name(const std::string& name) { __name = name; }
    const std::string& name() const { return __name; }

    Molecule& filter(const unsigned int hm = Residue::notassigned,
                     const std::string& chain_ids = "");
    Molecule& regenerate_bonds(const Molecule&);
    Molecule& compute_overlapping_rigid_segments(Unique&);

    void prepare_for_mm(const OMMIface::ForceField& ffield,
                        const Atom::Grid& grid);

    geometry::Coordinate compute_geometric_center() const {
        return geometry::compute_geometric_center(this->get_crds());
    }
    ...
    void add_bio_rota(const int& biomolecule_number, const int& rn,
                      const int& row, const geometry::Matrix::matrix_tuple& t) {
        __bio_rota[biomolecule_number][rn].set_row(
            row, t);  // needs copy constructor in geometry::Matrix
    }
    void add_bio_chain(const int& biomolecule_number,
                       std::set<char> bio_chain) {
        __bio_chain[biomolecule_number] = bio_chain;
    }
    Molecule& erase_properties() {
        for (auto& assembly : *this) assembly.erase_properties();
        return *this;
    }
